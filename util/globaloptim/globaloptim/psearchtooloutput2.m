function [stop,optnew,updateoptions] = psearchtooloutput2(optimval,optlast,flag)
%PSEARCHTOOLOUTPUT2 private to OPTIMTOOL.
...
switch flag
    ...
    case 'iter'
        ...
        switch RunMode
            ...
            case PAUSE  %Pause
                fprintf('%s\n%s\n','OPTIMTOOL is paused. MATLAB Command prompt will', ...
                    'not be accessible until the pattern search2 solver is completed.');
        ...
        if  javaMethodEDT('getChangedState',optimtoolGui) || modelCheck
            ...
            try
                if ~isempty(errOpt)
                    error(message('globaloptim:psearchtooloutput2:invalidModel', errOpt));
                end
                
                % Attempt to update changed options
                optnew = optimtoolUpdateChangedOptions2(...
                    optnew, optlast, 'patternsearch2');
                                
                % Indicate that the options have been updated
                updateoptions = true;
            catch ME
                errordlg(ME.message,'Pattern search2 run time error');
