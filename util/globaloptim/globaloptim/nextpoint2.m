function [found,nextIterate,FunEval] = nextpoint2(FUN,Xin,X,Iterate,FunEval,maxFun, ...
    CompleteSearch,NotVectorized,Cache,cachetol,cachelimit,varargin)
%NEXTPOINT2 Returns the best iterate. 	
%   [FOUND,NEXTITERATE,FUNEVAL] = NEXTPOINT2(FUN,X,ITERATE,FUNEVAL, ...
...
% 		
%   COMPLETESEARCH: If 'off' indicates that POLL2 can be called off as soon 
...
if NotVectorized
    for i = 1:maxeval   
        [f,count] = funevaluate2(FUN,Xin,X(:,i),Cache,cachetol,cachelimit,varargin{:});
...
elseif strcmpi(CompleteSearch,'on')
    [f,count] = funevaluate2(FUN,Xin,X(:,1:maxeval),Cache,cachetol,cachelimit,varargin{:});
    FunEval = FunEval+count;
    for i =1:maxeval
