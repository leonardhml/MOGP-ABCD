function [XOUT,Aineq,Bineq,Aeq,Beq,LB,UB,msg,exitflag] = preProcessLinearConstr2( ...
    XOUT,Aineq,Bineq,Aeq,Beq,LB,UB,TolCon,numberOfVariables,type,verbosity)
%PREPROCESSLINEARCONSTR2 validates dimension of constraint matrices, removes
%   redundancy in them, and finds initial feasible point.
%
%   private to PATTERNSEARCH2 and GA2.
...
if ~isempty(Aineq)
    if ~isequal(length(Bineq),nineqcstr)
        error(message('globaloptim:preprocesslinearconstr2:inconsistentAineqAndBineq'))
    elseif ~isequal(numberOfVariables,n)
        error(message('globaloptim:preprocesslinearconstr2:inconsistentAineqAndX0'))
    end

    % Check for empty constraints that are never satisfied (i.e. A(i,:) = 0 and b(i) < 0)
    zeroConstrID = all(Aineq == 0,2); % get rows with all zeros
    infeasZeroConstrID = zeroConstrID & (Bineq(:) < -TolCon);
    if any(infeasZeroConstrID)
        XOUT = Xin;
        msg = getString(message('globaloptim:preprocesslinearconstr2:infeasibleAineq'));
        exitflag = -2;
        return;
    end
    % Remove any other empty constraints
    if ~isempty(zeroConstrID)
        Aineq(zeroConstrID,:) = [];
        Bineq(zeroConstrID) = [];
    end
elseif ~isempty(Bineq)
    error(message('globaloptim:preprocesslinearconstr2:emptyAineqNotBineq'))
end
if ~isempty(Aeq)
    if ~isequal(length(Beq),neqcstr)
        error(message('globaloptim:preprocesslinearconstr2:inconsistentAeqAndBeq'))
    elseif ~isequal(numberOfVariables,m)
        error(message('globaloptim:preprocesslinearconstr2:inconsistentAeqAndX0'))
    end
    % Remove empty constraints.
    zeroConstrID = all(Aeq == 0,2); % get rows with all zeros
    % Check for empty constraints that are never satisfied (i.e. Aeq(i,:) = 0 and beq(i) ~= 0)
    infeasZeroConstrID = zeroConstrID & (abs(Beq(:)) > TolCon);
    if any(infeasZeroConstrID)
        XOUT = Xin;
        msg = getString(message('globaloptim:preprocesslinearconstr2:infeasibleAineq'));
        exitflag = -2;
    ...
    % Remove all other empty rows
    if ~isempty(zeroConstrID)
        Aeq(zeroConstrID,:) = [];
        Beq(zeroConstrID) = [];
    end
elseif ~isempty(Beq)
    error(message('globaloptim:preprocesslinearconstr2:emptyAeqNotBeq'))
end

% Remove dependent constraint, if any and find a basic solution
[XOUT,Aineq,Bineq,Aeq,Beq,msg,how,exitflag]= eqnsolv2(XOUT,Aineq,Bineq,Aeq,Beq,LB,UB,tol,verbosity);
...
feasible = isTrialFeasible2(XOUT,Aineq,Bineq,Aeq,Beq,[],[],tol);

% Not a feasible point? find an initial feasible point using LP (Two
% approaches)
if ~feasible
    % Suppress warnings that 'simplex' and 'active-set' will be removed
    origwarnings = warning('off', 'optim:linprog:AlgOptsWillError');
    ...
    feasible = isTrialFeasible2(XOUT,Aineq,Bineq,Aeq,Beq,LB,UB,tol);
    if success <=0 || ~feasible
        % Add a slack variable and find a feasible initial point
        [XOUT,success] = initialfeasible2(XOUT,numberOfVariables,Aineq,Bineq,Aeq,Beq,LB,UB);
        if success > 0
            feasible = isTrialFeasible2(XOUT,Aineq,Bineq,Aeq,Beq,LB,UB,tol);
        end
    end
