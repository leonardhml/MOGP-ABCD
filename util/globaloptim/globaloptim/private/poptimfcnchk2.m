function [Iterate,count] = poptimfcnchk2(FUN,nonlcon,Xin,Iterate,Vectorized,objFcnArg,conFcnArg)
%POPTIMFCNCHK2 Calls objective function 'FUN' and nonlinear constraint.
...
if strcmpi(Vectorized,'off')  % The function is not vectorized. 
    try
        [y,count] = funevaluate2(FUN,Xin,X,'init',[],[],objFcnArg{:});
    catch userFcn_ME
        gads_ME = MException('globaloptim:poptimfcnchk2:objfunCheck', ...
            'Failure in initial user-supplied objective function evaluation. PATTERNSEARCH2 cannot continue.');
        userFcn_ME = addCause(userFcn_ME,gads_ME);
        rethrow(userFcn_ME)
   end
    if numel(y) ~=1
        error(message('globaloptim:poptimfcnchk2:objfunCheck'));
    end
elseif strcmpi(Vectorized,'on') % If vectorized is 'on', Completepoll MUST be 'on' too
    X2 = [X, X];
    try
        [f,count] = funevaluate2(FUN,Xin,X2,'init',[],[],objFcnArg{:});
    catch userFcn_ME
        gads_ME = MException('globaloptim:poptimfcnchk2:objfunCheck', ...
            'Failure in initial user-supplied objective function evaluation (hint: ''vectorized'' option is ''on''). PATTERNSEARCH2 cannot continue.');
        userFcn_ME = addCause(userFcn_ME,gads_ME);
        rethrow(userFcn_ME)
    end
    if 2 ~=numel(f)
        error(message('globaloptim:poptimfcnchk2:objfunCheckVectorized', ... 
            'Vectorized','on'));
    end
    y = f(1);
end
...
if isnan(y)
    error(message('globaloptim:poptimfcnchk2:objfunNaN'));
end
Iterate.f = y;
...
if  ~isempty(nonlcon)
    try
        if strcmpi(Vectorized,'on')
            ...
            [tmpCineq,tmpCeq] = feval(nonlcon,reshapeinput2(Xin,X2),conFcnArg{:});            
        else
            [cineq,ceq] = feval(nonlcon,reshapeinput2(Xin,X),conFcnArg{:});
        end
    catch userFcn_ME
        gads_ME = MException('globaloptim:poptimfcnchk2:confunCheck', ...
            'Failure in initial user-supplied nonlinear constraint function evaluation. PATTERNSEARCH2 cannot continue.');
        userFcn_ME = addCause(userFcn_ME,gads_ME);
        rethrow(userFcn_ME)
    end
    if strcmpi(Vectorized,'on')
        ...
        if ( ~isempty(tmpCineq) && ( size(tmpCineq,2^isXinColumnVector) ~= 2 ) ) || ...
            ...
            if isXinColumnVector
                error(message('globaloptim:poptimfcnchk2:x0ColConfunCheckVectorized', ...
                    'Vectorized','on'));
            else
                error(message('globaloptim:poptimfcnchk2:x0RowConfunCheckVectorized', ...
                    'Vectorized','on'));
            end
        end
        cineq = [];
    ...
    if ~all(isreal(c) & isfinite(c))
        error(message('globaloptim:poptimfcnchk2:confunNotReal'));
    end
end