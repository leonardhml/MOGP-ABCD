function state = migrate2(FitnessFcn,GenomeLength,options,state )
%MIGRATE2 Migrate2 members between sub-populations.
%   state = migrate2(FitnessFcn,GenomeLength,options,state ); migrates individuals between
%   subpopulations.
%
...
%   Individuals that migrate2 from one sub-population to another are copied. 
...
%   into the first, and the first may migrate2 into the last. To prevent
...
subPops = populationIndices2(populations);
...
% How many will migrate2 from each sub-population?
...
for pop = 1:length(populations)
    ...
    if multiObj
        [~,i] = sortrows([state.Rank(p(1):p(2)) state.Distance(p(1):p(2))],[1,-2]);
    else
        [~,i] = sort(state.Score(p(1):p(2)));
    end
    sourcePop = i(:) + p(1) - 1;
...
for i = 1:length(populations)
    ...
    if multiObj
        state.C(to,:) = state.C(from,:);
        state.Ceq(to,:) = state.Ceq(from,:);
        state.isFeas(to) = state.isFeas(from,:);
        state.maxLinInfeas(to) = state.maxLinInfeas(from,:);
    end
end
