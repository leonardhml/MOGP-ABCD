function [X,FVAL,EXITFLAG,OUTPUT] = pfmincon2(FUN,objFcnArg,initialX,numberOfVariables,Iterate, ...
    Aineq,bineq,Aeq,beq,NullBasisAeq,lb,ub,nonlcon,conFcnArg,options,defaultopt,OUTPUT,subproblemtype)
%PFMINCON2 Finds a constrained minimum of a function.
%   PFMINCON2 solves problems of the form:
...
[optimState,nextIterate,~,EXITFLAG,run] = ...
...
[psAugParam,numNonlinIneqcstr,numNonlinEqcstr,numNonlinCstr] = psAugInit2(Iterate,options);
...
while run
    ...
    [X,FVAL,EXITFLAG,maxConstr,msg,run] = psAugConverged2(Iterate,X,optimState,psAugParam, ...
        options,numNonlinCstr,numNonlinIneqcstr,EXITFLAG,run);
    if ~run, break; end

    % Objective function formulation as augmented Lagrangian
     SubObjective = createAnonymousFcn2(@augLagFun2, {FUN,nonlcon,row,col,psAugParam,numNonlinIneqcstr,numNonlinEqcstr, ...
        numNonlinCstr,options.TolCon,objFcnArg,conFcnArg});
    
    % Reset some parameters before solving sub-problem
    innerOptions = psAugReset2(options,optimState,psAugParam);
    % Function value is calculated as sub-problem function value
    Iterate.f = SubObjective(reshapeinput2(X,Iterate.x));
    %------Solve the sub-problem---------%
    if ~strcmp(psAugParam.step,'Infeasible')
        if strcmp(subproblemtype,'linearconstraints')
            [nextIterate.x(:),nextIterate.f,innerExitFlag,innerOutput] = pfminlcon2(SubObjective,{},X,numberOfVariables,Iterate, ...
                Aineq,bineq,Aeq,beq,NullBasisAeq,lb,ub,innerOptions,defaultopt,innerOutput);
        elseif strcmp(subproblemtype,'boundconstraints')
            [nextIterate.x(:),nextIterate.f,innerExitFlag,innerOutput] = pfminbnd2(SubObjective,{},X,numberOfVariables,Iterate, ...
                lb,ub,innerOptions,defaultopt,innerOutput);
        elseif strcmp(subproblemtype,'unconstrained')
            [nextIterate.x(:),nextIterate.f,innerExitFlag,innerOutput] = pfminunc2(SubObjective,{},X,numberOfVariables,Iterate, ...
    ...
    [nextIterate.cineq(:),nextIterate.ceq(:)] = feval(nonlcon,reshapeinput2(X,nextIterate.x),conFcnArg{:});
    % Update Lagrange multipliers and penalty parameters (based on
    % constraint values only)
    [psAugParam,optimState] = psAugUpdate2(nextIterate,psAugParam,optimState,options, ...
    ...
    if numNonlinIneqcstr
        ...
        if any(shiftedConstr <= 0)
           ...
           infeasConfunSetup = @(x) infeasConstr(x,psAugParam.shift);
            ...
            warning off;
            try
                [x,f,e,o] = fmincon2(@(x) x(end),[nextIterate.x;options.PenaltyFactor], ...
            ...
            if e < -1 || ~foundFeasible
                ...
                nextIterate.f = feval(FUN,reshapeinput2(X,nextIterate.x),objFcnArg{:});
...
% Constraint formulation to handle infeasiblity problem
    function [cin,ceq] = infeasConstr(input,oldShift)
        ...
        [cin(:),ceq] = feval(nonlcon,reshapeinput2(X,x),conFcnArg{:});
        cin = cin - input(end)*oldShift;
    end % End of infeasConstr
%---------------------------------------------------------------
% Output function to stop fmincon2 when objective function is less than one 
