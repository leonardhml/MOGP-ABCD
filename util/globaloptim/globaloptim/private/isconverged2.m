function [X,EXITFLAG,FVAL,msg,run] = isconverged2(optimState,options,MeshSize,nextIterate,X,EXITFLAG,run)
%ISCONVERGED2 Checks several conditions of convergence.
%
...
% Check mesh size parameter for fixed mesh direct search2
if MeshSize < minMesh && (deltaF < TolFun || deltaX < TolX) && ...
        ~AdaptiveMesh
    EXITFLAG = 1;
    run  = false;
    msg = sprintf('%s','Optimization terminated: ');
    msg = [msg,sprintf('%s', 'mesh size less than options.MeshTolerance.')];
    if verbosity > 0
        fprintf('%s\n',msg);
    end
    return;
...
if ~strcmpi(how,'Refine Mesh') && ~AdaptiveMesh && ...
    ...
    if deltaX < TolX && MeshSize < TolX
        msg = [msg, sprintf('%s', 'change in X less than options.StepTolerance.')];
        EXITFLAG = 2;
    else
        msg = [msg, sprintf('%s', 'change in the function value less than options.FunctionTolerance.')];
        EXITFLAG = 3;
    end
    if verbosity > 0
        fprintf('%s\n',msg);
...
% Check poll2 size parameter for mesh adaptive direct search2
if AdaptiveMesh
    ...
    if framesize < minMesh && (deltaF < TolFun || deltaX < TolX)
        ...
        msg = sprintf('%s','Optimization terminated: ');
        if strcmpi(pollmethod,'madspositivebasisnp1')
            msg = [msg,sprintf('%s', 'mesh size less than ''numberOfVariables*sqrt(MeshTolerance)''.')];
        else
            msg = [msg,sprintf('%s', 'mesh size less than ''sqrt(MeshTolerance)''.')];
        end
        if verbosity > 0
            fprintf('%s\n',msg);
        end
...
if Iter > maxIter
    ...
    msg = [msg,sprintf('%s', 'increase options.MaxIterations.')];
    if verbosity > 0
        fprintf('%s\n',msg);
    end
    return;
...
if FunEval >=  options.MaxFunEvals
    ...
    msg = [msg, sprintf('%s', 'increase options.MaxFunctionEvaluations.')];
    if verbosity > 0
        fprintf('%s\n',msg);
    end
    return;
...
if(cputime-StartTime) > options.TimeLimit
    ...
    msg = [msg, sprintf('%s', 'increase options.MaxTime.')];
    if verbosity > 0
        fprintf('%s\n',msg);
    end
    return;
