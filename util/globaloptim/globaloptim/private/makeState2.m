function state = makeState2(GenomeLength,FitnessFcn,Iterate,type,options)
%MAKESTATE2 Create an initial population and fitness scores
%   state = makeState2(GenomeLength,FitnessFcn,options) Creates an initial 
   ...
   state.Score  =  zeros(totalPopulation,1);
    ...
    if strcmpi(options.Vectorized, 'off')
        try
            ...
            gads_ME = MException('globaloptim:makestate2:fitnessFcnFailed', ...
                'Failure in initial user-supplied fitness function evaluation. GA2 cannot continue.');
            userFcn_ME = addCause(userFcn_ME,gads_ME);
            rethrow(userFcn_ME)
        end
        % User-provided fitness function should return a scalar
        if numel(firstMemberScore) ~= 1
            error(message('globaloptim:makestate2:fitnessCheck'));
        end        
        Score = fcnvectorizer2(state.Population(initScoreProvided+2:end,:),FitnessFcn,1,options.SerialUserFcn);
        ...
        if numel(Score) ~= individualsToEvaluate
            error(message('globaloptim:makestate2:fitnessCheckVectorized', ...
                'Vectorized','on'));
        end
        state.Score(initScoreProvided+1:end) =  Score(:);
    end
