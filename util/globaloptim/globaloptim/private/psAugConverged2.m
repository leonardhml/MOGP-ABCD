function [X,FVAL,EXITFLAG,maxConstr,msg,run] = psAugConverged2(Iterate,X,optimState, ...
    psAugParam,options,numNonlinCstr,numNonlinIneqcstr,EXITFLAG,run)
%PSAUGCONVERGED2 Augmented Lagrangian barrier convergence test.
...
if deltaX <= TolX && currentTolMesh <= TolX && maxConstr <= TolCon
    EXITFLAG = 2;
    run  = false;
    msg = sprintf('%s','Optimization terminated: Change in X less than options.StepTolerance');
...
if ~isempty(infMessage) & strmatch('optimlib:optimfcnchk2',infMessage)
    error(message('globaloptim:psAugConverged2:NaNFval'));
end
