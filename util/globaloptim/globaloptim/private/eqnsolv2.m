function [X,Aineq,Bineq,Aeq,Beq,msg,how,exitflag]= eqnsolv2(X,Aineq,Bineq,Aeq,Beq,LB,UB,tolcon,verbosity)
%EQNSOLV2 Helper function for PFMINLCON2. 
...
if issparse(Aeq)
    warning(message('globaloptim:eqnsolv2:eqSparseToFull'));
    Aeq = full(Aeq);
    Beq = full(Beq);
end
if issparse(Aineq)
    warning(message('globaloptim:eqnsolv2:ineqSparseToFull'));
    Aineq = full(Aineq);
    Bineq  = full(Bineq);
end
A = [Aeq;Aineq];
...
if ~strcmp(how,'infeasible') && ACTCNT > 0
    if isTrialFeasible2(X,Aineq,Bineq,Aeq,Beq,LB,UB,tolcon)
      return;
    end
