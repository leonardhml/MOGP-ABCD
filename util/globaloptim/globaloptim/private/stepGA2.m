function [nextScore,nextPopulation,state] = stepGA2(thisScore,thisPopulation,options,state,GenomeLength,FitnessFcn)
%STEPGA2 Moves the genetic algorithm forward by one generation
...
if strcmpi(options.Vectorized, 'off') 
    nextScore = fcnvectorizer2(nextPopulation,FitnessFcn,1,options.SerialUserFcn);
else
