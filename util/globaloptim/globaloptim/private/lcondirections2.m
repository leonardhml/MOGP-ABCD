function [Basis,TangentCone] = lcondirections2(AdaptiveMesh,MeshSize,x,Aineq,bineq,Aeq,lb,ub,tol)
%LCONDIRECTIONS2 finds search2 vectors when linear constraints and bounds are
%   present. AdaptiveMesh is a boolean value which is true if MADS is used to poll2
...
if ~isempty(Aineq)
    while rank(Normals) ~= min(size(Normals))
        if tol < tolDep
            error(message('globaloptim:lcondirections2:degenconstr'));
        end
