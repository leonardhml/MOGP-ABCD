function [f,count] = funevaluate2(FUN,Xin,X,Cache,cachetol,cachelimit,varargin)
%FUNEVALUATE2 Evaluate objective function FUN at X.
...
if strcmpi(Cache,'init') || strcmpi(Cache,'off')  %No CACHE use
    XCache = [];
    count = size(X,2);
    f = feval(FUN,reshapeinput2(Xin,X),varargin{:});
...
elseif strcmpi(Cache,'on')
    ...
    for i = 1:size(X,2)
        for j = CacheSize:-1:1
            if isequalpoint2(X(:,i),XCache(:,j),cachetol)
                InCache(i)= 1;
                count = count - 1;
                break;
            end
    ...
    if any(~InCache)
        f(~InCache) = feval(FUN,reshapeinput2(Xin,X(:,~InCache)),varargin{:});
    end
    Real = isreal(f) & ~isnan(f);
    f(~Real) = NaN;
end
