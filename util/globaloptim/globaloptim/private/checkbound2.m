...
function [lb,ub,msg,exitflag] = checkbound2(lbin,ubin,nvars)
%CHECKBOUND2 Move the initial point within the (valid) bounds.
%   [X,LB,UB,X,FLAG] = checkbounds2(X0,LB,UB,nvars) checks that the upper and lower
...
if lenlb > nvars
       warning(message('globaloptim:checkbound2:lengthOfLowerBound'));
       lb = lb(1:nvars);   
    lenlb = nvars;
elseif lenlb < nvars
...
if lenub > nvars
       warning(message('globaloptim:checkbound2:lengthOfUpperBound'));
       ub = ub(1:nvars);
    lenub = nvars;
elseif lenub < nvars
...
if any(eq(ub, -inf)) 
   error(message('globaloptim:checkbound2:infUpperBound'));
elseif any(eq(lb,inf))
   error(message('globaloptim:checkbound2:infLowerBound'));
end
