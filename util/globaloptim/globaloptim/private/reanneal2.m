function solverData = reanneal2(solverData,problem,options)
% REANNEAL2 is used to turn our SIMULANNEALBND2 algorithm into an adaptive
...
if strcmpi(options.DataType, 'double')
    ...
    % Set inputs to finitedifferences2
    Diff = zeros(nvar,1); % pre-allocate vector containing derivatives
    sizes.xShape = size(problem.x0);
    ...
    finDiffFlags.chkFunEval = false; % Don't validate2 function values
    ...
    [Diff,~,~,numEvals] = finitedifferences2(solverData.currentx,FUN,[],lb,ub, ...
