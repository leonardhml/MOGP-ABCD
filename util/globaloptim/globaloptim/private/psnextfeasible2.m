function [msg,nextIterate,direction,FunEval] = psnextfeasible2(ObjFunc,Xin,sites,Iterate, ...
    Aineq,bineq,Aeq,beq,lb,ub,tol,constr,objFcnArg,FunEval,options)
%PSNEXTFEASIBLE2 finds next feasible iterate among 'sites' in a pattern.
%   Input argument 'objfunc' is the objective function on which POLL2 step
%   is implemented, 'sites' is a set of points generated by POLL2 technique.
...
%   If POLL2 is unsuccessful 'nextIterate' is same as the input argument 'Iterate'.
%
%   The successful search2 direction vector is returned as 'direction' and number of
...
for k = 1:length(sites)
    ...
    sites(k).f = inf;
    if constr
        feasible(k) = isTrialFeasible2(sites(k).x,Aineq,bineq,Aeq,beq,lb,ub,tol);
    end
end
sites(~feasible) = [];
...
if options.NotVectorizedPoll
    if strcmpi(options.CompletePoll,'on')
        ...
        if options.SerialUserFcn
            for k = 1:maxeval
                trialF(k) = funevaluate2(ObjFunc,Xin,trialX(:,k),Cache, ...
                    CacheTol,CacheSize,objFcnArg{:});
            end
        else
            parfor (k = 1:maxeval)
                % Do not use Cache when running parfor
                trialF(k) = funevaluate2(ObjFunc,Xin,trialX(:,k),'off', ...
                    [],[],objFcnArg{:});
            end
        end
        ...
        for k = 1:maxeval
            [sites(k).f,count] = funevaluate2(ObjFunc,Xin,sites(k).x,Cache, ...
                CacheTol,CacheSize,objFcnArg{:});
...
elseif strcmpi(options.CompletePoll,'on')
    ...
    if constr
        [feasible,Xfeas] = getFeasiblePoints2([sites(1:maxeval).x],Aineq,bineq,Aeq,beq,lb,ub,tol);
    end
    [f,count] = funevaluate2(ObjFunc,Xin,Xfeas,options.Cache,options.CacheTol, ...
