function state = gadsplot2(options,optimvalues,flag,fname)
%GADSPLOT2 Helper function that manages the plot functions.
%   STOP = GADSPLOT2(options,optimvalues,flag,fname) runs
%   each of the plot functions.
%
%   This function is private to simulanneal2, patternsearch2, ga2, gamultiobj2.
...
% Different for GA2 and other solvers
if strcmpi(fname,'genetic algorithm')
  state = optimvalues;
  currentIter = optimvalues.Generation;
elseif any(strcmpi(fname, {'multistart2', 'globalsearch2'}))
  % Note that there are no spaces in the fname for MultiStart2 and
  % GlobalSearch2, as this is how these solvers are referred to in the
  % product documentation.
  state = false;
  currentIter = optimvalues.localrunindex;
  % GlobalSearch2 and MultiStart2 do not support PlotInterval and
  % PlotFcnsArgs. Set them to sensible defaults.
  options.PlotInterval = 1;
  options.PlotFcnsArgs = repmat({{}}, 1, length(options.PlotFcns));
elseif strcmpi(fname, 'particleswarm2')
  state = false;
  currentIter = optimvalues.iteration;  
  % particleswarm2 does not support PlotInterval and
  % PlotFcnsArgs. Set them to sensible defaults.
  options.PlotInterval = 1;
  options.PlotFcnsArgs = repmat({{}}, 1, length(options.PlotFcns));  
else
...
switch lower(strtrim(fname))
  case {'pattern search2','multistart2','globalsearch2','particleswarm2'}
...
if rows1 == rows && cols1 == cols
  %Binary search2 and replacement
  for i = 1:length(plotNames)
    if plotNo(i) == 0
      for j = 1:rows*cols
        if ~any(j == plotNo)
