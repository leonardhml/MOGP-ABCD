function [X,FVAL,EXITFLAG,output,POPULATION,SCORES] = gacon2(FitnessFcn,GenomeLength, ...
    Aineq,bineq,Aeq,beq,lb,ub,NonconFcn,options,output,Iterate,subtype)
%GACON2 Genetic algorithm generalized constrained solver.
%   Private function to GA2
%
%  GACON2 solves problems of the form:
...
[gaAugParam,numNonlinIneqcstr,numNonlinEqcstr,numNonlinCstr] = gaAugInit2(Iterate,options);
Iterate.f = FitnessFcn(Iterate.x');

% Parameter to track if nonlcon returns non-real value to fmincon2
infMessage = [];
...
SubFitness = createAnonymousFcn2(@augLagFun2,{FitnessFcn,NonconFcn,1,GenomeLength, ...
...
% ALGA.
state = makeState2(GenomeLength,SubFitness,Iterate,subtype,options);
...
state = gadsplot2(options,state,'init','Genetic Algorithm');
[state,options] = gaoutput2(FitnessFcn,options,state,'init');
...
while run
    ...
    [X,FVAL,EXITFLAG,maxConstr,output.message,run] = gaAugConverged2(options,state,gaAugParam,Iterate, ...
    ...
    innerOptions = gaAugReset2(innerPopulation,options,state,gaAugParam);
    ...
    if ~strcmp(gaAugParam.step,'Infeasible')
        if any(strcmp(subtype,{'boundconstraints','linearconstraints'}))
            [~,Iterate.f,innerExitFlag,innerOutput,innerPopulation,innerScore] = ...
                galincon2(SubFitness,GenomeLength,Aineq,bineq,Aeq,beq,lb,ub,innerOptions,innerOutput,Iterate);
        elseif strcmp(subtype,'unconstrained')
            [~,Iterate.f,innerExitFlag,innerOutput,innerPopulation,innerScore] = ...
                gaunc2(SubFitness,GenomeLength,innerOptions,innerOutput,Iterate);
        end
    end
    ...
    [gaAugParam,state] = gaAugUpdate2(Iterate,gaAugParam,state,options,numNonlinIneqcstr,numNonlinCstr);
    ...
    if numNonlinIneqcstr
        ...
        if any(shiftedConstr <= 0)
            ...
            warning off;
            try
                [x,f,e,o] = fmincon2(@(x) x(end),[Iterate.x;options.PenaltyFactor], ...
    ...
    state = gadsplot2(options,state,'iter','Genetic Algorithm');
    [state,options,optchanged] = gaoutput2(FitnessFcn,options,state,'iter');
    if optchanged
...
gadsplot2(options,state,'done','Genetic Algorithm');
gaoutput2(FitnessFcn,options,state,'done');
...
%-----------------------------------------------------------------
    function obj = getFitnessValue(pop)
        ...
        if strcmpi(options.Vectorized, 'off')
            obj = fcnvectorizer2(pop,FitnessFcn,1,options.SerialUserFcn);
        else
            obj = FitnessFcn(pop);
        end
    end
