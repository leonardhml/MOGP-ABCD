function [msg,nextIterate,direction,FunEval] = nextfeasible2(ObjFunc,nonlcon,Xin,sites,Iterate,A,LB,UB ...
    ,FunEval,maxFun,Completepoll,TolBind,IndEqcstr,NotVectorized,Cache,cachetol,cachelimit,conFcnArg,varargin)
%NEXTFEASIBLE2 Finds next feasible iterate in a POLL2 step.
% 	This function is private to POLL2 and SEARCH2 functions.
% 	
% 	OBJFUNC: The objective function on which POLL2 step is implemented.
% 	
% 	SITES: A set of design points generated by POLL2 technique. We would like
...
% 	
% 	DIRECTION: Successful POLL2 direction. This information can be used 
% 	by POLL2 techniques in ordering the search2 direction (last successful
...
if NotVectorized
    for k = 1:maxeval
        ...
        sites(k).f = inf;
        if constr
            feasible  = isfeasible2(sites(k).x,A,LB,UB,TolBind,IndEqcstr);
        end
        %Calculate nonlinear constraints if linear constraints and bounds
        %are satisfied
        if feasible && nonlinconstr
            [cineq,ceq] = feval(nonlcon,reshapeinput2(Xin,sites(k).x),conFcnArg{:});
            cineq = max(cineq); ceq = max(abs(ceq));
            feasible = cineq <= TolBind & ceq < TolBind;
        end
        if feasible % All the constraints are satisfied
            [sites(k).f,count] = funevaluate2(ObjFunc,Xin,sites(k).x,Cache,cachetol,cachelimit,varargin{:});
...
elseif strcmpi(Completepoll,'on')  % vectorized is 'on', Completepoll MUST be 'on' too
    ...
    if constr
        [Xfeas,feasible] = allfeasible2([sites(1:maxeval).x],A,LB,UB,TolBind,IndEqcstr);
    end
    ...
    [f,count] = funevaluate2(ObjFunc,Xin,Xfeas,Cache,cachetol,cachelimit,varargin{:});
