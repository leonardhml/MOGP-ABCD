function [x,fval,exitFlag,output,population,scores] = gamultiobj2(fun,nvars,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options)
%GAMULTIOBJ2 Multiobjective optimization using genetic algorithm.
%   GAMULTIOBJ2 attempts to solve multiobjective problems of the form:
%       min F(X)  subject to:  A*X <= b, Aeq*X = beq (linear constraints)
%        X                     lb <= X <= ub (bound constraints)
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS) finds a local Pareto set X of the
...
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS,A,b) finds a local Pareto set X of the
...
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS,A,b,Aeq,beq) finds a local Pareto set X
...
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS,A,b,Aeq,beq,lb,ub) defines a set of
...
%   supported.
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS,A,b,Aeq,beq,lb,ub,NONLCON) finds a local 
...
%
%   X = GAMULTIOBJ2(FITNESSFCN,NVARS,A,b,Aeq,beq,lb,ub,NONLCON,options)
%   finds a Pareto set X with the default optimization parameters replaced
%   by values in OPTIONS. OPTIONS can be created with the optimoptions2
%   function. See optimoptions2 for details. For a list of options accepted
%   by GAMULTIOBJ2 refer to the documentation.
%
%   X = GAMULTIOBJ2(PROBLEM) finds the minimum for PROBLEM. PROBLEM is a
...
%          options: <Options created with optimoptions2('gamultiobj2',...)>
%           solver: <solver name 'gamultiobj2'>
%         rngstate: <State of the random number generator>
%
%   [X,FVAL] = GAMULTIOBJ2(FITNESSFCN,NVARS, ...) in addition returns a
%   matrix
...
%   of rows as does X.
%
%   [X,FVAL,EXITFLAG] = GAMULTIOBJ2(FITNESSFCN,NVARS, ...) in addition returns
%   EXITFLAG which describes the exit condition of GAMULTIOBJ2. Possible values
...
%
%   [X,FVAL,EXITFLAG,OUTPUT] = GAMULTIOBJ2(FITNESSFCN,NVARS, ...) in addition
...
%             message: <GAMULTIOBJ2 termination message>
%
%   [X,FVAL,EXITFLAG,OUTPUT,POPULATION] = GAMULTIOBJ2(FITNESSFCN, ...) in
%   addition returns the final POPULATION at termination. 
%
%   [X,FVAL,EXITFLAG,OUTPUT,POPULATION,SCORE] = GAMULTIOBJ2(FITNESSFCN, ...)
%   in addition returns the SCORE of the final POPULATION.
%
%
%   Example:
%    Multiobjective minimization of two functions 'ackleyfcn2' and 'shufcn2'
%
%      fun1 = @(x) ackleyfcn2(x);
%      fun2 = @(x) shufcn2(x);
...
%      options = optimoptions2('gamultiobj2','InitialPopulationRange',[lb;ub]);
%      % Minimize using GAMULTIOBJ2
%      [x,fval] = gamultiobj2(fun1and2,2,[],[],[],[],lb,ub,options)
%
%    Display Pareto fronts and rank of individuals while GAMULTIOBJ2
%    minimizes 
%
%      options = optimoptions2('gamultiobj2','InitialPopulationRange',[lb;ub]);
%      options = optimoptions2(options,'PlotFcn',{@gaplotpareto2,@gaplotrankhist2});
%      [x,fval,exitflag,output] = gamultiobj2(fun1and2,2,[],[],[],[],lb,ub,options)
%
%
%   See also optimoptions2, PATTERNSEARCH2, GA2, @.
...
defaultopt = struct('PopulationType', 'doubleVector', ...
    ...
    'SelectionFcn', {{@selectiontournament2,2}}, ...
    'CrossoverFcn',@crossoverintermediate2, ...
    'MutationFcn',@mutationadaptfeasible2, ...
    'DistanceMeasureFcn',{{@distancecrowding2, 'phenotype'}}, ...
    'HybridFcn',[], ...
    'Display', 'final', ...
    'PlotFcns', [], ...
    'OutputFcns', [], ...
...
try 
    ...
    error(message('globaloptim:gamultiobj2:numberOfInputs', ME.message));
end
...
if ~isempty(nonlcon) && (isstruct(nonlcon) || isa(nonlcon, 'optim.options.SolverOptions2')) && nargin < 10
    options = nonlcon;
    nonlcon = [];
end
...
if nargin == 1
    if isa(fun,'struct')
        ...
        error(message('globaloptim:gamultiobj2:invalidStructInput'));
    end
end

% Prepare the options for the solver
options = prepareOptionsForSolver2(options, 'gamultiobj2');
...
if isempty(FitnessFcn) ||  ~(isa(FitnessFcn,'inline') || isa(FitnessFcn,'function_handle'))
    error(message('globaloptim:gamultiobj2:needFunctionHandle'));
end
...
valid = isnumeric(nvars) && isscalar(nvars) && (nvars > 0) ...
    && (nvars == floor(nvars));
if ~valid
    error(message('globaloptim:gamultiobj2:notValidNvars'));
end
...
if ~isempty(options) && ~isa(options,'struct')
        error(message('globaloptim:gamultiobj2:optionsNotAStruct'));
...
msg = isoptimargdbl2('GAMULTIOBJ2', {'NVARS','A',   'b',   'Aeq','beq','lb','ub'}, ...
                                    nvars,  Aineq, bineq, Aeq,  beq,  lb,  ub);
if ~isempty(msg)
    error('globaloptim:gamultiobj2:dataType',msg);
end

% Introduce field to describe the objective type
options.MultiObjective = true;

% Validate2 constraints and options
[x,fval,exitFlag,output,population,scores,FitnessFcn,nvars,Aineq,bineq,Aeq,beq,lb,ub, ...
    ~,options] = gacommon2(nvars,fun,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,[],options,user_options);
...
[x,fval,exitFlag,output,population,scores] = gamultiobjsolve2(FitnessFcn,nvars, ...
     Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options,output);
