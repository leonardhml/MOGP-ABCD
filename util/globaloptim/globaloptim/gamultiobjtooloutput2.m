function [state,optnew,updateoptions]  = gamultiobjtooloutput2(optlast,state,flag)
%GAMULTIOBJTOOLOUTPUT2 private to OPTIMTOOL. 
    ...
    case 'iter'
        ...
        switch RunMode
            ...
            case PAUSE  %Pause
                ...
                %tolerances. GAMULTIOBJ2 does not have LastImprovementTime
        ...
        if  javaMethodEDT('getChangedState',optimtoolGui) || modelCheck
            ...
            try 
                if ~isempty(errOpt)
                    error(message('globaloptim:gamultiobjtooloutput2:invalidModel', errOpt));
                end
                ...
                optnew = optimtoolUpdateChangedOptions2(...
                    optnew, optlast, 'gamultiobj2', extradefault, validatefcn);
                
                % Indicate that the options have been updated
                updateoptions = true;
            catch ME
                errordlg(ME.message,'GAMULTIOBJ2 run time error');    
...
% Validate2 the newly obtained option
...
optnewstructwithdefault = validate2(optnewstructwithdefault,type,...
