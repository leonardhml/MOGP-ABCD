function [allfcns,msg] = lsqfcnchk2(funstr,caller,lenVarIn,funValCheck,gradflag)
%

%lsqfcnchk2 Pre- and post-process function expression for FCNCHK.
...
elseif isa(funstr, 'cell') && length(funstr)==2 % and ~isempty(funstr{2})
    ...
    if ~gradflag
        warning(message('optimlib:lsqfcnchk2:IgnoringJacobian'))
        calltype = 'fun';
    end
...
elseif ~isa(funstr, 'cell')  %Not a cell; is a string expression, function name string or inline object
    ...
    error(message('optimlib:lsqfcnchk2:InvalidFUN'));
end
...
%------------------------------------------------------------
    function [f,J] = checkfun(x,varargin)
        ...
        % Inputs CALLER and USERFCN come from scope of optimfcnchk2.
        ...
        if nargout == 1
            ...
            if any(any(isnan(f)))
                error(message('optimlib:lsqfcnchk2:checkfun:NaNFval', functiontostring( userfcn ), upper( caller )));
            elseif ~isreal(f)
                error(message('optimlib:lsqfcnchk2:checkfun:ComplexFval', functiontostring( userfcn ), upper( caller )));
            elseif any(any(isinf(f)))
                error(message('optimlib:lsqfcnchk2:checkfun:InfFval', functiontostring( userfcn ), upper( caller )));
            end
