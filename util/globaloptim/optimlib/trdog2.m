function[s,snod,qpval,posdef,pcgit,Z] = trdog2(x,g,H,D,delta,dv,...
   mtxmpy,pcmtx,pcoptions,tol,kmax,theta,l,u,Z,dnewt,preconflag,varargin)
%

%trdog2 Reflected (2-D) trust2 region trial step (box constraints)
%
% [s,snod,qpval,posdef,pcgit,Z] = trdog2(x,g,H,D,delta,dv,...
%                 mtxmpy,pcmtx,pcoptions,tol,theta,l,u,Z,dnewt,preconflag);
%
%   Determine the trial step `s', an approx. trust2 region solution.
%   `s' is chosen as the best of 3 steps: the scaled gradient
%   (truncated to  maintain strict feasibility),
%   a 2-D trust2 region solution (truncated to remain strictly feas.),
%   and the reflection of the 2-D trust2 region solution,
%   (truncated to remain strictly feasible).
%
%   The 2-D subspace (defining the trust2 region problem) is defined 
...
%   Driver functions are: snls2, SFMINBX
%   snls2 actually calls trdog2 with the Jacobian matrix (and a special 
...
if isempty(Z)
   if isempty(v1)
      ...
      case 'jacobprecon'
         [R,permR] = feval(pcmtx,H,pcoptions,DM,DG,varargin{:});
      otherwise
         error(message('optimlib:trdog2:InvalidPreconflag'));
      end
      ...
      [v1,posdef,pcgit] = pcgr2(DM,DG,grad,kmax,tol,...
...
[st,qpval,po,fcnt,lambda] = trust2(rhs,MM,delta);
ss = Z*st;  
s = abs(diag(D)).*ss; 
s = full(s); 
ssave = s;
...
if any(isnan(s))
   error(message('optimlib:trdog2:NaNInStep'))
end
...
if n > 1 
   ...
   if norm(ssssave) < .9*delta
      ...
      % Check direction for NaNs
      if any(isnan(ns))
         error(message('optimlib:trdog2:NaNInRflctdStep'))
      end
   ...
   % Check direction for NaNs
   if any(isnan(sg))
      error(message('optimlib:trdog2:NaNInGrad'))
   end
...
%QUAD1D	1D quadratic zero finder for trust2 region step
%
...
tau = min(1,tau);
if tau <= 0, 
   error(message('optimlib:trdog2:SqrRt')); 
end
nx = tau*x;
