function [xcurr,fvec,LAMBDA,JACOB,EXITFLAG,OUTPUT,msgData] = snls2(funfcn,xstart,l,u,verb,options, ...
    defaultopt,fval,JACval,caller,Jstr,computeLambda,mtxmpy,detailedExitMsg,optionFeedback,finDiffFlags,varargin)
%

%snls2  Sparse nonlinear least squares solver.
%   
...
%
% x=snls2(fname,xstart) solves the unconstrained nonlinear least
% squares problem. The vector function is 'fname' and the
% starting point is xstart. 
%
% x=snls2(fname,xstart,options) solves the unconstrained or
...
% options.
%
% x=snls2(fname,xstart,options,Jstr) indicates the structure
% of the sparse Jacobian matrix -- sparse finite differencing
% is used to compute the Jacobian when Jstr is not empty.
%
% [x,fvec] =snls2(fname,xstart, ...)  returns the final value of the
% vector function F.
%
% [x,fvec,gopt] = snls2(fname,xstart, ...) returns the first-order
% optimality vector.
%
% [x,fvec,gopt,iter] = snls2(fname,xstart, ...) returns the number of
% major iterations used.
%
% [x,fvec,gopt,iter,npcg] =  snls2(fname,xstart, ...) returns the
% total number of CG iterations used.
%
% [x,fvec,gopt,iter,npcg,ex] = snls2(fname,xstart, ...) returns the 
% termination code.
...
if any(~isfinite(fval)) 
    error(message('optimlib:snls2:UsrObjUndefAtX0', caller));    
end
if any(~isfinite(nonzeros(JACval)))
    error('optimlib:snls2:UserJacUndefAtX0', ...
...
if n == 0, 
   error(message('optimlib:snls2:InvalidN'))
end
...
if min(u-l) <= 0
   error(message('optimlib:snls2:InconsistentBounds'))
end

%
numberOfVariables = n;
% Read in user options 
% Add Preconditioner to defaultopt. This is not a documented option, and is
% not added to defaultopt at the user-facing function level.
defaultopt.Preconditioner = @aprecon2;
...
if ischar(kmax)
   if isequal(lower(kmax),'max(1,floor(numberofvariables/2))')
      ...
      error(message('optimlib:snls2:InvalidMaxPCGIter'))
   end
end
if ischar(maxfunevals)
   if isequal(lower(maxfunevals),'100*numberofvariables')
      ...
      error(message('optimlib:snls2:MaxFunEvals'))
   end
end
...
if ~gradflag % use sparse finite differencing
   ...
   if any(~isfinite(nonzeros(A)))
        error('optimlib:snls2:DerivUndefAtX0', ...
...
[mm,pp] = size(fvec);
if mm < n, 
   error(message('optimlib:snls2:MLessThanN'))
end
...
if haveoutputfcn || haveplotfcn
    ...
    if stop
        ...
        msgData = {'snls2',EXITFLAG,verb > 0,detailedExitMsg,caller};
        return;
    end
end
...
while ~ex 
   ...
   if ((optnrm < tol1) && (posdef == 1) )
       ...
       msgData = {'snls2',msgFlag,verb > 0,detailedExitMsg,caller, ...
   ...
   elseif iter > itb,
       ex = 4; EXITFLAG = 0;
       msgData = {'snls2',10,verb > 0,detailedExitMsg,caller, ...
   ...
   if ~ex 
      ...
      %       Determine the trust2 region correction
      dd = abs(v); D = sparse(1:n,1:n,full(sqrt(dd)));
      theta = max(.95,1-optnrm);  
      oposdef = posdef;
      [sx,snod,qp,posdef,pcgit,Z] = trdog2(x,g,A,D,delta,dv,...
      ...
      %       Perturb?
      [~,newx] = perturbTrustRegionReflective2(newx,l,u);
      ...
      if ~gradflag %use sparse finite-differencing
         ...
         otherwise
            error(message('optimlib:snls2:UndefinedCalltype', caller))
         end
         [newA, findiffevals] = sfdnls2(xcurr,newfvec,Jstr,group,[], ...
                       funfcn,l,u,options,sizes,finDiffFlags,varargin{:});
      ...
      faultTolStruct = updateFaultTolStruct2(faultTolStruct,newval,verb > 1);
      
      % Update trust2 region radius
      if ~faultTolStruct.currTrialWellDefined
          % Shrink the trust2 region if any element of the function vector
...
if computeLambda
    % The gradient of the least squares function can be written as
    % 
    % grad = 2JACOB'fval = 2g (using the definition of g in this file)
    %
    
    LAMBDA = formBoxLambda2(2*g,l,u);
else
    LAMBDA = [];
end
%--------------------------------------------------------------------------
...
if ~isempty(outputfcn)
    switch state
        ...
        otherwise
            error(message('optimlib:snls2:UnknownStateInCALLOUTPUTANDPLOTFCNS'))
    end
end
% Call plot functions
if ~isempty(plotfcns)
