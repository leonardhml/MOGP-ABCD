function checkoptionsize2(option, optionsize, numvars, numfunelts)
%

%checkoptionsize2 Verify problem size dependent options 
%   checkoptionsize2('OPTION', OPTIONSIZE, NUMVARS) verifies that the
...
%   numel(X0)).
%
%   checkoptionsize2('JacobPattern', OPTIONSIZE, NUMVARS, NUMFUNELTS)
...
switch lower(option)
    case 'typicalx'
        if prod(optionsize) ~= numvars
            ME = MException('optimlib:checkoptionsize2:InvalidSizeOfTypicalX', ...
    ...
    case 'hesspattern'
        ...
        if ~isequal(optionsize, expsize)
            ME = MException('optimlib:checkoptionsize2:InvalidSizeOfHessPattern', ...
    ...
    case 'jacobpattern'
        ...
        if ~isequal(optionsize, expsize)
            ME = MException('optimlib:checkoptionsize2:InvalidSizeOfJacobPattern', ...
