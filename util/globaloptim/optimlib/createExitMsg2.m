function outputMsg = createExitMsg2(caller,exitflag,dispMsg,detailedExitMsg,varargin)
%

%createExitMsg2 create exit messages for Optimization Toolbox solvers.
%
...
% array. That is, there must be a uniform amount of inputs to createExitMsg2.
...
if enableLinks
    ...
    detailTag = sprintf(['<<a href = "matlab: createExitMsg2(''%s'',%e,true,true,''%s'',%e,''%s'',%e,%e,''%s'',%e,%e,''%s'',%e);">' ...
...
lsqLocalOptTag = sprintf(linkCmd,'local_min_lsq');              % exitflag 1: levenbergMarquardt2,snls2
...
lsqInitPtTag = sprintf(linkCmd,'lsq_init_pt_min');              % exitflag 100: levenbergMarquardt2,snls2
normGradTag = sprintf(linkCmd,'first_order_opt_unc');           % exitflag 1,100: fminusub, sfminbx, sfminle,levenbergMarquardt2
gradSizeTag = sprintf(linkCmd,'grad_size');                     % exitflag 1,100: fminusub, sfminbx, sfminle
regProbTag = sprintf(linkCmd,'appears_regular');                % exitflag 1,100: levenbergMarquardt2,snls2,trustnleqn
lsqGradSizeTag = sprintf(linkCmd,'lsq_grad_size');              % exitflag 1,100: levenbergMarquardt2,snls2
lsqNormGradTag = sprintf(linkCmd,'firstorder_lsq');             % exitflag 1,100: levenbergMarquardt2,snls2
eqSolvInitTag = sprintf(linkCmd,'eq_init_pt');                  % exitflag 100: levenbergMarquardt2,snls2,trustnleqn
eqSolvedTag = sprintf(linkCmd,'eqn_solved');                    % exitflag 1: levenbergMarquardt2,snls2,trustnleqn
...
eqSolveSingTag = sprintf(linkCmd,'eqn_solved_singular');        % exitflag 2: levenbergMarquardt2,snls2,trustnleqn (fsolve)
lsqPossMinTag = sprintf(linkCmd,'local_min_possible_lsq');      % exitflag 2: levenbergMarquardt2,snls2,trustnleqn (lsqnonlin/curvefit)
conLocalOptTag = sprintf(linkCmd,'local_min_found');            % exitflag 1: nlconst2,barrier,sqpLineSearch
conInitPtTag = sprintf(linkCmd,'did_not_move');                 % exitflag 100: barrier,sqpLineSearch
firstOrdOptTag = sprintf(linkCmd,'first_order_opt_con');        % exitflag 1,100: barrier,sqpLineSearch,nlconst2
feasDirTag = sprintf(linkCmd,'feasible_directions');            % exitflag 1,100: barrier,sqpLineSearch,nlconst2
...
normSearchDirTag = sprintf(linkCmd,'norm_search_direction');    % exitflag 4,-24: nlconst2
...
jacCalcFailTag = sprintf(linkCmd,'lsq_no_Jacobian');            % exitflag 26: levenbergMarquardt2
...
eqSolvedInaccTag = sprintf(linkCmd,'eqn_inaccuracy');           % exitflag 3: levenbergMarquardt2,snls2,trustnleqn (fsolve)
ineffectiveStepTag = sprintf(linkCmd,'last_step_ineffective');  % exitflag 3: levenbergMarquardt2,snls2,trustnleqn (fsolve)
inFeasTag = sprintf(linkCmd,'no_feasible_point');               % exitflag -20,-21,-22,-24,-25: barrier,sqpLineSearch,nlconst2
...
noEqSolTag = sprintf(linkCmd,'eqn_no_soln');                    % exitflag -21,-22,-23,-24: trustnleqn,levenbergMarquardt2,snls2
noEqSolTrrTag = sprintf(linkCmd,'eqn_no_soln_negexit');         % exitflag -3: trustnleqn,levenbergMarquardt2,snls2
...
maxSQPIterTag = sprintf(linkCmd,'maxsqpiter_csh');              % exitflag -20: nlconst2
...
constrTolTag = sprintf(linkCmd,'constraint_tolerance');         % All exitflags: barrier,sqpLineSearch,nlconst2
tolConTag = sprintf(linkCmd,'tol_con');                         % All exitflags: barrier,sqpLineSearch,nlconst2
...
switch exitflag
    case stoppedAtX0 % exitflag = 100
        switch caller
            case {'barrier','sqpLineSearch'}
                % messageInfo = {'fmincon2',firstordopt,tolFunChoice,tolFun,maxConstr,tolConChoice,tolCon}
            ...
            case {'trustnleqn','snls2'}
                ...
                else % caller is lsqnonlin/lsqcurvefit2
        ...
        switch caller
            ...
            case {'sfminbx','sfminle'}
                ...
                detailedMsg = sprintf(['\nOptimization completed: The %sfirst-order optimality measure%s', ...
                    ...
                    'detected in the trust2-region model.\n\n', ...
                    'Optimization Metric                                        Options\n', ...
                    'relative first-order optimality = %10.2e   OptimalityTolerance = %7.0e (%s)\n'],normGradTag,endLinkTag, ...
                    messageInfo{2},optimalityTolFminuncTag,endLinkTag,messageInfo{4},messageInfo{2},messageInfo{4},messageInfo{3});
            case 'fminusub'
            ...
            case {'snls2','trustnleqn'}
                if strcmpi(messageInfo{1},'fsolve')
                    ...
                    detailedMsg = sprintf(['\nEquation solved. The sum of squared function values, r = %e, ', ...
                        ...
                        messageInfo{2},messageInfo{4},messageInfo{3},messageInfo{5},messageInfo{7},messageInfo{6});
                else % Caller is lsqnonlin or lsqcurvefit2
                    % messageInfo = {'lsq*',firstOrdOpt,tolFunChoice,tolFun}
                    basicMsg = sprintf(['\n%sLocal minimum found%s.\n\n',...
                        'Optimization completed because the %ssize of the gradient%s is less than\n',...
                        'the %s value of the %soptimality tolerance%s.\n\n%s\n'], ...
    ...
    case deltaX % exitflag = 2
        switch caller
            case {'barrier','sqpLineSearch'}
                % Create messages for display
                % messageInfo = {'fmincon2',normStep,tolXchoice,tolX,maxConstr,tolConChoice,tolCon}
    ...
    case undefGrads % exitflag = 26
        switch caller
            case {'barrier','sqpLineSearch'}
                % messageInfo = {'fmincon2',maxConstr,tolConChoice,tolCon}
                basicMsg = sprintf(['\n%sLocal minimum possible. Constraints satisfied%s.\n\n',...
    ...
    case deltaF % exitflag = 3
        switch caller
            case {'sfminbx','sfminle'}
                % messageInfo = {'fmincon2',relnorm(deltaF),tolFunChoice,tolFun}
                basicMsg = sprintf(['\n%sLocal minimum possible%s.\n\n',...
            ...
            case {'snls2','levenbergMarquardt2'}
                if strcmpi(messageInfo{1},'fsolve')
    ...
    case smallSearchDir % exitflag = 4
        switch caller
            case 'nlconst2'
                % messageInfo = {'fmincon2',normSD,tolXchoice,tolX,maxConstr,tolConChoice,tolCon}
                basicMsg = sprintf(['\n%sLocal minimum possible. Constraints satisfied%s.\n\n',...
    ...
    case dirDeriv % exitflag = 5
        switch caller
            case 'nlconst2'
                % Create messages for display
                % messageInfo = {'fmincon2',abs(alpha*dirDeriv),tolFunChoice,tolFun,maxConstr,tolConChoice,tolCon}
                basicMsg = sprintf(['\n%sLocal minimum possible. Constraints satisfied%s.\n\n',...
    ...
    case infeasSQP % exitflag = -20
        % Create messages for display - caller is nlconst2
        % messageInfo = {'fmincon2',[],maxSQPIterChoice,maxSQPIter,maxConstr,tolConChoice,tolCon}
    ...
    case infeasDirDeriv % exitflag = -25
        % Create messages for display - caller is nlconst2
        % messageInfo = {'fmincon2',alpha*dirDeriv,tolXchoice,tolX,maxConstr,tolConChoice,tolCon}
        basicMsg = sprintf(['\n%sConverged to an infeasible point%s.\n\n',...
            '%s stopped because the %spredicted change in the objective function%s\n', ...
    ...
    case unbounded % exitflag = -3
        switch caller
            case {'barrier','sqpLineSearch'}
                % messageInfo = {'fmincon2',fval,objLimitChoice,objLimit,maxConstr,tolConChoice,tolCon}
            ...
            case 'trustnleqn'
                ...
                detailedMsg = sprintf(['\n%s stopped because the %strust-region radius%s, %e, is less than 2*eps.\n\n' ...
                    'Optimization Metric                                             Options\n', ...
                    'trust2-region radius = %10.2e                              2*eps = %8.1e\n'],messageInfo{1}, ...
    ...
    case nonconvex % exitflag = -6
        ...
        detailedExitMsg = false;
    otherwise
        basicMsg = sprintf('\nOptimization stopped.\nUnrecognized exitflag input to createExitMsg2.\n');
end
