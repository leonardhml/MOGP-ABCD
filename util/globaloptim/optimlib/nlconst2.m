function [x,FVAL,lambda_out,EXITFLAG,OUTPUT,GRADIENT,HESS]= ...
    nlconst2(funfcn,x,lb,ub,Ain,Bin,Aeq,Beq,confcn,OPTIONS,defaultopt,...
    finDiffFlags,verbosity,flags,initVals,problemInfo,optionFeedback,varargin)
%

%nlconst2 Helper function to find the constrained minimum of a function
%   of several variables. Called by fmincon2, FGOALATTAIN, FSEMINF, and
%   FMINIMAX.
...
if isFseminf
    ...
    serialFinDiff = ~validateopts_UseParallel(UseParallel,true,true);
end
...
% Add undocumented options to defaultopt before checking for user-set values
defaultopt.NoStopIfFlatInfeas = 'off';
defaultopt.PhaseOneTotalScaling = 'off';
...
% In case the defaults were gathered from calling: optimset('fmincon2'):
if ischar(maxFunEvals)
    if isequal(lower(maxFunEvals),'100*numberofvariables')
        maxFunEvals = 100*numberOfVariables;
    else
        error(message('optimlib:nlconst2:InvalidMaxFunEvals'))
    end
end
...
sizes.xShape = size(x); % input to finitedifferences2()
sizes.nVar = numberOfVariables;
sizes.mNonlinIneq = non_ineq;  
sizes.mNonlinEq = non_eq;
...
% Indices for finitedifferences2() call. Make them column vectors
...
% the caller is fmincon2 (see g210993).
...
if isempty(f)
    error(message('optimlib:nlconst2:InvalidFUN'))
end
...
% and before passing them to qpsub2---which would error otherwise.
...
% Create a QP options structure for qpsub2
...
if ischar(qpmaxiter)
    if isequal(lower(qpmaxiter),'10*max(numberofvariables,numberofinequalities+numberofbounds)')
        ...
        error(message('optimlib:nlconst2:InvalidMaxSQPIter'))
    end
end    
...
else % fmincon2 or fseminf is caller
    header = ...
...
while ~done
    ...
    if constflag && ~flags.gradconst || ~flags.grad
        % If there are nonlinear constraints and their gradients are not
        % supplied, or the objective gradients are not supplied, or
        ...
        if serialFinDiff         % Call serial finite-differences code
            if ~isFseminf
                if (~flags.grad && ~flags.gradconst) 
                    % No objective gradients, no constraint gradients
                    [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
                        finitedifferences2(XOUT,funfcn{3},confcn{3},lb,ub,f, ...
                        nc(nonlIneqs_idx),nc(nonlEqs_idx),1:numberOfVariables, ...   
                        OPTIONS,sizes,gf,gnc(:,nonlIneqs_idx), ...
                        gnc(:,nonlEqs_idx),finDiffFlags,[],varargin{:});
                elseif ~flags.gradconst % No constraint gradients; objective
                    % gradients supplied
                    [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
                        finitedifferences2(XOUT,[],confcn{3},lb,ub,f,nc(nonlIneqs_idx), ...
                        nc(nonlEqs_idx),1:numberOfVariables,OPTIONS,sizes,gf, ...
                ...
                elseif ~flags.grad % No objective gradients, constraint gradients supplied
                    [gf,~,~,numEvals] = finitedifferences2(XOUT,funfcn{3},[],lb,ub,f,[],[], ...
                        1:numberOfVariables,OPTIONS,sizes,gf,[],[],finDiffFlags,[],varargin{:});
                end
            else
                if (~flags.grad && ~flags.gradconst) 
                    % No objective gradients, no constraint gradients
                    [gf,gnc,NEWLAMBDA,OLDLAMBDA,s,numEvals] = finDiffFseminf2(XOUT,x,funfcn, ...
                        lb,ub,f,nc,OPTIONS,'all',LAMBDA,NEWLAMBDA, ...
                        OLDLAMBDA,POINT,FLAG,s,startnlineq,varargin{:});
                    gnc = gnc'; % nlconst2 requires the transpose of the Jacobian
                elseif ~flags.gradconst 
                    % No constraint gradients; objective gradients supplied
                    [gf,gnc,NEWLAMBDA,OLDLAMBDA,s,numEvals] = finDiffFseminf2(XOUT,x,[], ...
                        lb,ub,f,nc,OPTIONS,'all',LAMBDA,NEWLAMBDA, ...
                        OLDLAMBDA,POINT,FLAG,s,startnlineq,varargin{:});
                    gnc = gnc'; % nlconst2 requires the transpose of the Jacobian
                end
          ...
          % The objective and constraint function handle are not passed to
          % parfinitedifferences2. They are available to
          % parfinitedifferences2 function from the local workers. See
          % optimlib/setOptimFcnHandleOnWorkers2.m for more information)
            if (~flags.grad && ~flags.gradconst) 
                % No objective gradients, no constraint gradients
                [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
                    parfinitedifferences2(XOUT,lb,ub,f, ...
                    nc(nonlIneqs_idx),nc(nonlEqs_idx),1:numberOfVariables, ...
                    OPTIONS,sizes,gf,gnc(:,nonlIneqs_idx), ...
                    gnc(:,nonlEqs_idx),finDiffFlags,[],varargin{:});
            elseif ~flags.gradconst 
                % No constraint gradients; objective gradients supplied
                [gf,gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),numEvals] = ...
                    parfinitedifferences2(XOUT,lb,ub,f,nc(nonlIneqs_idx), ...
                    nc(nonlEqs_idx),1:numberOfVariables,OPTIONS,sizes,gf, ...
                    gnc(:,nonlIneqs_idx),gnc(:,nonlEqs_idx),finDiffFlags,[],varargin{:});
            elseif ~flags.grad    % No objective gradients, constraint gradients supplied
                [gf,~,~,numEvals] = parfinitedifferences2(XOUT,lb,ub,f,[],[], ...
                    1:numberOfVariables,OPTIONS,sizes,gf,[],[],finDiffFlags,[],varargin{:});
            end
        end 
    ...
    if iter > 0 % All but 0th iteration ----------------------------------------
        ...
        if noStopIfFlatInfeas
            noFurtherProgress = ( abs(steplength)*max(abs(SD)) < 2*tolX || (abs(prodGFSD) < 2*tolFunValue && ...
        ...
        if optimError < tolFun*optimScal && feasError < tolCon*feasScal
            ...
            outMessage = createExitMsg2(nlconst2,EXITFLAG,verbosity > 1,flags.detailedExitMsg,caller, ...
            ...
            if verbosity > 1  % If the display is active, find active inequalities               
                if displayActiveInequalities
                    ...
                    if any(activeLb) || any(activeUb) || any(activeIneqLin) || any(activeIneqNonlin)
                        fprintf(getString(message('optimlib:nlconst2:ActiveIneqs',sprintf('%g',tolCon))))
                        disp('  lower      upper     ineqlin   ineqnonlin')
                        printColumnwise(activeLb,activeUb,activeIneqLin,activeIneqNonlin);
                    else
                        disp(getString(message('optimlib:nlconst2:NoActiveIneqs')))
                    end
                end
            end
        elseif noFurtherProgress
            [outMessage,EXITFLAG,lambdaNLP,optimError] = testConvergence(mg,SD,tolX,tolFun,tolFunValue,tolCon,...
            ...
            if numFunEvals > maxFunEvals
                ...
                outMessage = createExitMsg2(nlconst2,EXITFLAG,verbosity > 0,flags.detailedExitMsg,caller, ...
                    [],optionFeedback.MaxFunEvals,maxFunEvals);
            end
            if iter >= maxIter
                EXITFLAG = 0;
                done = true;
                % Call createExitMsg2 with exitflag = 10, for MaxIter violation
                outMessage = createExitMsg2(nlconst2,10,verbosity > 0,flags.detailedExitMsg,caller, ...
    ...
    if ~done
        ...
        [SD,lambda,exitflagqp,outputqp,howqp,ACTIND] ...
            = qpsub2(HESS,gf,AN,-GT,[],[],XN,eq,-1, ...
            nlconst2,size(AN,1),numberOfVariables,qpoptions,ACTIND,phaseOneTotalScaling);
        ...
        while runLineSearch && numFunEvals < maxFunEvals
            ...
            if hasBoundOnStep && (iter <= relLineSrchBndDuration)
                ...
                % for all i.
                indxViol = abs(trialsteplength*SD) > relLineSrchBnd*max(abs(MATX),abs(OPTIONS.TypicalX));
                if any(indxViol)
                    trialsteplength = sign(trialsteplength)*min(  min( abs(trialsteplength), ...
                        relLineSrchBnd*max(abs(MATX(indxViol)),abs(OPTIONS.TypicalX(indxViol))) ...
                        ./abs(SD(indxViol)) )  );
                end
            end
            ...
            % worth continuing
            if ~noStopIfFlatInfeas
                if ( norm(SD,Inf) < 2*tolX || abs(trialsteplength*prodGFSD) < tolFunValue ) && ...
        ...
        if ~done
            ...
            switch funfcn{1} % evaluate function gradients
                ...
                case 'fun_then_grad'
                    ...
                    numGradEvals = numGradEvals + 1;
                otherwise
                    error(message('optimlib:nlconst2:UndefinedCalltypeInFMINCON'));
            end
        ...
        end % if ~done (convergence check after qpsub2)
    end % if ~done (optimality check after qpsub2 AND linesearch)
end % while ~done
...
if strcmp(caller,'fmincon2') || strcmp(caller,'fseminf')
    optimValues.fval = f;
...
if ~isempty(outputfcn)
    switch state
        ...
        otherwise
            error(message('optimlib:nlconst2:UnknownStateInCALLOUTPUTANDPLOTFCNS'))
    end
end
% Call plot functions
if ~isempty(plotfcns)
...
% CLEANUPINTERRUPT updates or sets all the output arguments of nlconst2 when the optimization
...
if strcmp(caller,'fmincon2') || strcmp(caller,'fseminf')
    ...
    % fgoalattain and fminimax expect that nlconst2 return
...
OUTPUT.message = createExitMsg2('nlconst2',EXITFLAG,verbosity > 0,detailedExitMsg,caller);
...
% and bounds.
% INPUT:
% c                 vector of constraints and bounds (see nlconst2 main code)
% tol               tolerance to determine when an inequality is active
% arglb, argub      boolean vectors indicating finite bounds (see nlconst2
%                   main code)
...
% instead of '<= tol' to be onsistent with nlconst2 main code,
...
%--------------------------------------------------------------------------
function [outMessage,EXITFLAG,lambda,optimError] = testConvergence(mg,SD,tolX,tolFun,tolFunValue,tolCon,...
    verbosity,displayActiveInequalities,c,arglb,argub,lin_eq,non_eq,eq,ncstr,Ain,flags,...
...
if mg < tolCon
    ...
    if optimError < tolFun*optimScal
        EXITFLAG = 1;
        outMessage = createExitMsg2('nlconst2',EXITFLAG,verbosity > 1,detailedExitMsg,caller, ...
    ...
    elseif norm(SD,Inf) < 2*tolX
        ...
        EXITFLAG = 5;
        outMessage = createExitMsg2('nlconst2',EXITFLAG,verbosity > 1,detailedExitMsg,caller, ...
            dirDeriv,optionFeedback.TolFunValue,tolFunValue,mg,optionFeedback.TolCon,tolCon);
    end
    ...
    if verbosity > 1
        if displayActiveInequalities % If the display is active, find active inequalities
            ...
            [activeLb,activeUb,activeIneqLin,activeIneqNonlin] = ...
                activeInequalities(c,tolCon,arglb,argub,lin_eq,non_eq,size(Ain));
            if any(activeLb) || any(activeUb) || any(activeIneqLin) || any(activeIneqNonlin)
                fprintf(getString(message('optimlib:nlconst2:ActiveIneqs', ...
                    sprintf('%g',tolCon) )));
                disp('  lower      upper     ineqlin   ineqnonlin')
                printColumnwise(activeLb,activeUb,activeIneqLin,activeIneqNonlin);
            else
    ...
    if strcmp(howqp,'MaxSQPIter')
        % Call createExitMsg2 with createExitMsgexitflag = -20 for MaxSQPIter and infeasible
        outMessage = createExitMsg2('nlconst2',-20,verbosity > 0,detailedExitMsg,caller, ...
            [],optionFeedback.MaxSQPIter,maxSQPIter,mg,optionFeedback.TolCon,tolCon);
    elseif norm(SD,Inf) < 2*tolX
        % Call createExitMsg2 with createExitMsgexitflag = -24 for small search direction and infeasible
        outMessage = createExitMsg2('nlconst2',-24,verbosity > 0,detailedExitMsg,caller, ...
            norm(SD,Inf),optionFeedback.TolX,tolX,mg,optionFeedback.TolCon,tolCon);
    else
        % Call createExitMsg2 with createExitMsgexitflag = -25 for small directional derivative and infeasible
        outMessage = createExitMsg2('nlconst2',-25,verbosity > 0,detailedExitMsg,caller, ...
            dirDeriv,optionFeedback.TolFunValue,tolFunValue,mg,optionFeedback.TolCon,tolCon);
    end
    EXITFLAG = -2;
