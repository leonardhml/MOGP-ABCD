function [gradf,cJac,NEWLAMBDA,OLDLAMBDA,s,numEvals] = finDiffFseminf2(xCurrent,...
...
if fwdFinDiff % forward differences
    CHG = FinDiffRelStep.*nonzerosign2(xCurrent).*max(abs(xCurrent),abs(typicalx));
...
%
CHG = nonzerosign2(CHG).*min(max(abs(CHG),DiffMinChange),DiffMaxChange);
...
if fwdFinDiff % forward differences
    for gcnt = variables
        if (nonEmptyLowerBounds && isfinite(lb(gcnt))) || (nonEmptyUpperBounds && isfinite(ub(gcnt)))
            ...
            CHG(gcnt) = fwdFinDiffInsideBnds2(xCurrent(gcnt),lb(gcnt),ub(gcnt),CHG(gcnt),gcnt,DiffMinChange);
        end 
    ...
    for gcnt = variables
        if (nonEmptyLowerBounds && isfinite(lb(gcnt))) || (nonEmptyUpperBounds && isfinite(ub(gcnt)))
            % Enforce bounds while finite-differencing; may need to modify step
            [CHG(gcnt),formulaType] = cntrlFinDiffInsideBnds2(xCurrent(gcnt),lb(gcnt),ub(gcnt),CHG(gcnt),gcnt,DiffMinChange);
        else
        ...
        if len_cCurrent~=length(cPert1)
            cPert1 = v2sort2(cCurrent,cPert1);
        end
        ...
        if len_cCurrent~=length(cPert2)
            cPert2 = v2sort2(cCurrent,cPert2);
        end
        
        % Compute finite difference gradients
        if ~isempty(funfcn)
            gradf(gcnt,1) = twoStepFinDiffFormulas2(formulaType,CHG(gcnt),fCurrent,fPert1,fPert2);
        end
        if ~isempty(cJac)
            if ~isempty(cPert1) && ~isempty(cPert2)
                cJac(:,gcnt) = twoStepFinDiffFormulas2(formulaType,CHG(gcnt),cCurrent,cPert1,cPert2);
            end
        end
