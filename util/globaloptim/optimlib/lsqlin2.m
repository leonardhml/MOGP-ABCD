function [X,resnorm,residual,exitflag,output,lambda] = lsqlin2(C,d,A,b,Aeq,beq,lb,ub,X0,options,varargin)
%lsqlin2 Constrained linear least squares.
%   X = lsqlin2(C,d,A,b) attempts to solve the least-squares problem
%
...
%   where C is m-by-n.
%
%   X = lsqlin2(C,d,A,b,Aeq,beq) solves the least-squares
%   (with equality constraints) problem:
%
%           min  0.5*(NORM(C*x-d)).^2    subject to 
%            x                               A*x <= b and Aeq*x = beq
%
%   X = lsqlin2(C,d,A,b,Aeq,beq,LB,UB) defines a set of lower and upper
...
%
%   X = lsqlin2(C,d,A,b,Aeq,beq,LB,UB,X0) sets the starting point to X0.
%
%   X = lsqlin2(C,d,A,b,Aeq,beq,LB,UB,X0,OPTIONS) minimizes with the default
...
%
%   X = lsqlin2(PROBLEM) solves the least squares problem defined in 
...
%   structure in PROBLEM.options, and solver name 'lsqlin2' in 
%   PROBLEM.solver. Use this syntax to solve at the command line a problem 
%   exported from OPTIMTOOL. 
%
%   [X,RESNORM] = lsqlin2(C,d,A,b) returns the value of the squared 2-norm 
%   of the residual: norm(C*X-d)^2.
%
%   [X,RESNORM,RESIDUAL] = lsqlin2(C,d,A,b) returns the residual: C*X-d.
%
%   [X,RESNORM,RESIDUAL,EXITFLAG] = lsqlin2(C,d,A,b) returns an EXITFLAG
%   that describes the exit condition. Possible values of EXITFLAG and the
%   corresponding exit conditions are
%  
%     1  lsqlin2 converged to a solution X.
...
%         conditioned.
%
%   [X,RESNORM,RESIDUAL,EXITFLAG,OUTPUT] = lsqlin2(C,d,A,b) returns a 
...
%
%   [X,RESNORM,RESIDUAL,EXITFLAG,OUTPUT,LAMBDA] = lsqlin2(C,d,A,b) returns 
...
if nargin == 1
    if isa(C,'struct')
        ...
        error(message('optimlib:lsqlin2:InputArg'));
    end
end

% Prepare the options for the solver
[options, optionFeedback] = prepareOptionsForSolver2(options, 'lsqlin2');

if nargin == 0 
  error(message('optimlib:lsqlin2:NotEnoughInputs'))
end
% Check for non-double inputs
msg = isoptimargdbl2('lsqlin2', {'C','d','A','b','Aeq','beq','LB','UB','X0'}, ...
                                C,  d,  A,  b,  Aeq,  beq,  lb,  ub,  X0);
if ~isempty(msg)
    error('optimlib:lsqlin2:NonDoubleInput',msg);
end
...
trustRegion = 'trust2-region-reflective'; 
...
if ~any(strcmpi(output.algorithm, {activeSet,trustRegion,interiorPoint}))
    error(message('optimlib:lsqlin2:InvalidAlgorithm'));
end
algAndLargeScaleConflict = false;
if strcmpi(output.algorithm,trustRegion) && ~largescale
    % Conflicting options Algorithm='trust2-region-reflective' and
...
% Check if name clash
functionNameClashCheck2('JacobMult',mtxmpy,'atamult2','optimlib:lsqlin2:JacobMultNameClash');
...
if isempty(C) || isempty(d)
   error(message('optimlib:lsqlin2:FirstTwoArgsEmpty'))
else
...
[rows,cols]=size(C);
if length(d) ~= rows
   error(message('optimlib:lsqlin2:InvalidCAndD'))
end

if length(b) ~= size(A,1)
   error(message('optimlib:lsqlin2:InvalidAAndB'))
end

if length(beq) ~= size(Aeq,1)
   error(message('optimlib:lsqlin2:InvalidAeqAndBeq'))
end

if ( ~isempty(A)) && (size(A,2) ~= cols) 
    error(message('optimlib:lsqlin2:CAndA'))
end

if ( ~isempty(Aeq)) && (size(Aeq,2) ~= cols)
    error(message('optimlib:lsqlin2:CAndAeq'))
end

if isempty(X0) && ~strcmpi(output.algorithm,interiorPoint)
    % This zero-valued X0 will potentially be changed in sllsbox or qpsub2.
...
[X0,lb,ub,msg] = checkbounds2(X0,lb,ub,numberOfVariables);
if ~isempty(msg)
   exitflag = -2;
...
elseif strcmpi(output.algorithm,trustRegion)
    if algAndLargeScaleConflict
        warning(message('optimlib:lsqlin2:AlgAndLargeScaleConflict'));
        output.algorithm = activeSet;
    elseif (rows < cols)
        warning(message('optimlib:lsqlin2:MoreColsThanRows'));
        output.algorithm = activeSet;
    elseif hasLinearConstr
        warning(message('optimlib:lsqlin2:LinConstraints'));
        output.algorithm = activeSet;
    end
end
...
if diagnostics
   ...
   msg = diagnose2('lsqlin2',output,gradflag,hessflag,constflag,gradconstflag,...
...
case trustRegion
   ...
   output.message = msg;
case activeSet   
   % Sparse inputs are not accepted by qpsub2 
   if issparse(A) || issparse(C) || issparse(Aeq) 
      warning(message('optimlib:lsqlin2:ConvertToFull'))
   end    
   % Create options structure for qpsub2
   ...
   [X,lambdaqp,exitflag,output,~,~,msg]= ...
      qpsub2(full(C),d,[full(Aeq);full(A)],[beq;b],lb,ub,X0,neqcstr, ...
            verbosity,caller,ncstr,numberOfVariables,lsqlinoptions);
   output.algorithm = activeSet; % qpsub2 overwrites output with output.iterations 
case interiorPoint
...
if any(strcmpi(output.algorithm , {activeSet, interiorPoint, unconstrained}))
    ...
    if strcmpi(output.algorithm, activeSet)
        ...
        if exitflag == 1 % qpsub2 terminated successfully
    ...
    if any(strcmpi(output.algorithm , {activeSet, interiorPoint})) && ...
        ...
        output.firstorderopt = computeKKTErrorForQPLP2(H,f,A,b,Aeq,beq,lb,ub,lambda,X);
    end  
    output.cgiterations = [];
end
