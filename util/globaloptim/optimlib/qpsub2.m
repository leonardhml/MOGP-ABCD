function [X,lambda,exitflag,output,how,ACTIND,msg] = ...
    qpsub2(H,f,A,B,lb,ub,X,neqcstr,verbosity,caller,ncstr, ...
    numberOfVariables,options,ACTIND,phaseOneTotalScaling)
%

%qpsub2 solves quadratic programming problems. 
%
%   X = qpsub2(H,f,A,b) solves the quadratic programming problem:
%
...
if ncstr > neqcstr
    ...
    % tolCon option is used only by nlconst2.m. Other callers use tolcon = eps
    if ~isempty(tolCon)
...
if output.constrviolation > tolCon_scaled
     quiet = -2;
    % Feasible point finding phase for qpsub2
    % Use default options for qpsub2 in phase 1 
    ...
    [XS,lambdaS,exitflagS,outputS,howS,ACTIND2] = ...
        qpsub2([],[zeros(numberOfVariables,1);1],A2,[B;1e-5], ...
        [],[],[X;output.constrviolation+1],neqcstr,quiet,qpsub2,size(A2,1),numberOfVariables+1, ...
...
if (is_qp)
    gf=H*X+f;
    %  SD=-Z*((Z'*H*Z)\(Z'*gf));
    [SD, dirType] = compdir2(Z,H,gf,numberOfVariables,f);
        ...
        if strcmp(dirType, NewtonStep)
            ...
            if (~is_qp && ~LLS) || strcmp(dirType, NegCurv) % LP or neg def (implies is_qp)
                ...
                if is_qp && (norm(projH*projSD+Zgf) > 10*eps*(norm(projH) + norm(Zgf)))
                    % system is incompatible --> it's a "chute": use SD from compdir2
    ...
    if output.constrviolation > 1e5*errnorm
        if output.constrviolation > norm(X) * errnorm
            ...
            if exitflag == 1 && verbosity > 0 && ~illCondWarningAlreadyDisplayed
                warning(message('optimlib:qpsub2:SolUnreliable'))
...
% EQNSOLV Helper function for qpsub2.
