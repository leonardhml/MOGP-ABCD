function [SD, dirType] = compdir2(Z,H,gf,nvars,f);
%

% compdir2 computes a search direction in a subspace defined by Z.  
% [SD,dirType] = compdir2(Z,H,gf,nvars,f) returns a search direction for the
% subproblem 0.5*Z'*H*Z + Z'*gf. Helper function for nlconst2. SD is Newton
...
if ~p  % positive definite: use Newton direction
    ...
    if ~isempty(sneg) & sneg'*projH*sneg < -sqrt(eps) % if negative enough
        ...
        stpDesc = - Z*(Z'*gf);
        % ||SD|| may be (close to) zero, but qpsub2 handles that case
        SD = stpDesc;
