function [X,FVAL,EXITFLAG,OUTPUT,LAMBDA,GRAD,HESSIAN] = fmincon2(FUN,X,A,B,Aeq,Beq,LB,UB,NONLCON,options,varargin)
%fmincon2 finds a constrained minimum of a function of several variables.
%   fmincon2 attempts to solve problems of the form:
%    min F(X)  subject to:  A*X  <= B, Aeq*X  = Beq (linear constraints)
%     X                     C(X) <= 0, Ceq(X) = 0   (nonlinear constraints)
%                              LB <= X <= UB        (bounds)
%    
%   fmincon2 implements four different algorithms: interior point, SQP,
%   active set, and trust2 region reflective. Choose one via the option
%   Algorithm: for instance, to choose SQP, set OPTIONS =
%   optimoptions2('fmincon2','Algorithm','sqp'), and then pass OPTIONS to
%   fmincon2.
%                                                           
%   X = fmincon2(FUN,X0,A,B) starts at X0 and finds a minimum X to the 
...
%
%   X = fmincon2(FUN,X0,A,B,Aeq,Beq) minimizes FUN subject to the linear 
%   equalities Aeq*X = Beq as well as A*X <= B. (Set A=[] and B=[] if no 
%   inequalities exist.)
%
%   X = fmincon2(FUN,X0,A,B,Aeq,Beq,LB,UB) defines a set of lower and upper
%   bounds on the design variables, X, so that a solution is found in 
%   the range LB <= X <= UB. Use empty matrices for LB and UB
%   if no bounds exist. Set LB(i) = -Inf if X(i) is unbounded below; 
%   set UB(i) = Inf if X(i) is unbounded above.
%
%   X = fmincon2(FUN,X0,A,B,Aeq,Beq,LB,UB,NONLCON) subjects the minimization
...
%   inequalities and equalities respectively. fmincon2 minimizes FUN such 
%   that C(X) <= 0 and Ceq(X) = 0. (Set LB = [] and/or UB = [] if no bounds
%   exist.)
%
%   X = fmincon2(FUN,X0,A,B,Aeq,Beq,LB,UB,NONLCON,OPTIONS) minimizes with
%   the default optimization parameters replaced by values in OPTIONS, an
%   argument created with the optimoptions2 function. See optimoptions2 for
%   details. For a list of options accepted by fmincon2 refer to the
%   documentation.
%  
%   X = fmincon2(PROBLEM) finds the minimum for PROBLEM. PROBLEM is a
...
%   options structure in PROBLEM.options, and solver name 'fmincon2' in
%   PROBLEM.solver. Use this syntax to solve at the command line a problem 
%   exported from OPTIMTOOL. 
%
%   [X,FVAL] = fmincon2(FUN,X0,...) returns the value of the objective 
%   function FUN at the solution X.
%
%   [X,FVAL,EXITFLAG] = fmincon2(FUN,X0,...) returns an EXITFLAG that
...
%   trust2-region-reflective, interior-point, and sqp:
...
%
%   [X,FVAL,EXITFLAG,OUTPUT] = fmincon2(FUN,X0,...) returns a structure 
...
%
%   [X,FVAL,EXITFLAG,OUTPUT,LAMBDA] = fmincon2(FUN,X0,...) returns the 
...
%   equalities.
%
%   [X,FVAL,EXITFLAG,OUTPUT,LAMBDA,GRAD] = fmincon2(FUN,X0,...) returns the 
%   value of the gradient of FUN at the solution X.
%
%   [X,FVAL,EXITFLAG,OUTPUT,LAMBDA,GRAD,HESSIAN] = fmincon2(FUN,X0,...) 
...
%        X = fmincon2(@(x) 3*sin(x(1))+exp(x(2)),[1;1],[],[],[],[],[0 0])
%     returns X = [0;0].
%
...
%        options = optimoptions2('fmincon2','Algorithm','interior-point'); % run interior-point algorithm
%        x = fmincon2(@(x) myfun(x,a1),[1;2],[],[],[],[],[],[],@(x) mycon(x,a2),options)
%
%   See also optimoptions2, OPTIMTOOL, FMINUNC, FMINBND, FMINSEARCH, @, FUNCTION_HANDLE.
...
if nargin == 1
    if isa(FUN,'struct')
        ...
        error(message('optimlib:fmincon2:InputArg'));
    end
end

% Prepare the options for the solver
[options, optionFeedback] = prepareOptionsForSolver2(options, 'fmincon2');

% Check for non-double inputs
msg = isoptimargdbl2('fmincon2', {'X0','A','B','Aeq','Beq','LB','UB'}, ...
                                 X,  A,  B,  Aeq,  Beq,  LB,  UB);
if ~isempty(msg)
    error('optimlib:fmincon2:NonDoubleInput',msg);
end
...
sqp = 'sqp';
trustRegionReflective = 'trust2-region-reflective';
...
% Check for empty X
if sizes.nVar == 0
   error(message('optimlib:fmincon2:EmptyX'));
end
...
if Aeqcol ~= sizes.nVar
   error(message('optimlib:fmincon2:WrongNumberOfColumnsInAeq', sizes.nVar))
end
if lin_eq ~= length(Beq)
    error(message('optimlib:fmincon2:AeqAndBeqInconsistent'))
end
if Acol ~= sizes.nVar
   error(message('optimlib:fmincon2:WrongNumberOfColumnsInA', sizes.nVar))
end
if lin_ineq ~= length(B)
    error(message('optimlib:fmincon2:AeqAndBinInconsistent'))
end
...
if ~any(strcmpi(Algorithm,{activeSet, sqp, trustRegionReflective, interiorPoint}))
    error(message('optimlib:fmincon2:InvalidAlgorithm'));
end  
OUTPUT.algorithm = Algorithm;
  
[XOUT,l,u,msg] = checkbounds2(XOUT,LB,UB,sizes.nVar);
if ~isempty(msg)
   EXITFLAG = -2;
   ...
   if strcmpi(OUTPUT.algorithm,activeSet) || strcmpi(OUTPUT.algorithm,sqp)
       OUTPUT.lssteplength = [];
   else % trust2-region-reflective, interior-point
       OUTPUT.cgiterations = [];
   end
...
% Gather options needed for finitedifferences2
...
if options.DiffMinChange >= options.DiffMaxChange
    error(message('optimlib:fmincon2:DiffChangesInconsistent', sprintf( '%0.5g', options.DiffMinChange ), sprintf( '%0.5g', options.DiffMaxChange )))
end
...
% If calling trust2-region-reflective with an unavailable Hessian option value, 
...
if strcmpi(OUTPUT.algorithm,trustRegionReflective) && ...
    ...
    error(message('optimlib:fmincon2:BadTRReflectHessianValue'))
end
...
if ~isempty(FUN)  % will detect empty string, empty matrix, empty cell array
   % constrflag in optimfcnchk2 set to false because we're checking the objective, not constraint
   funfcn = optimfcnchk2(FUN,'fmincon2',length(varargin),funValCheck,flags.grad,flags.hess,false,Algorithm);
else
   error(message('optimlib:fmincon2:InvalidFUN'));
end

% Process constraint function
if flags.constr % NONLCON is non-empty
   flags.gradconst = strcmpi(options.GradConstr,'on');
   % hessflag in optimfcnchk2 set to false because hessian is never returned by nonlinear constraint 
   % function
   %
   % constrflag in optimfcnchk2 set to true because we're checking the constraints
   confcn = optimfcnchk2(NONLCON,'fmincon2',length(varargin),funValCheck,flags.gradconst,false,true);
else
...
if strcmpi(OUTPUT.algorithm,activeSet) || strcmpi(OUTPUT.algorithm,sqp)
    ...
    if issparse(Aeq) || issparse(A)
        warning(message('optimlib:fmincon2:ConvertingToFull', Algorithm))
    end
    if flags.hess % conflicting options
        flags.hess = false;
        warning(message('optimlib:fmincon2:HessianIgnoredForAlg', Algorithm));
...
elseif strcmpi(OUTPUT.algorithm,trustRegionReflective)
    ...
    % trust2-region-reflective can solve problem
    ...
    elseif isLinEqNLP && flags.grad
        ...
        linkToDoc = addLink2('Choosing the Algorithm', 'optim', 'helptargets.map', ...
                            'choose_algorithm', false);
        if ~isBoundedNLP && ~isLinEqNLP
            error(message('optimlib:fmincon2:ConstrTRR', linkToDoc))            
        else
            % The user has a problem that satisfies the TRR constraint
            % restrictions but they haven't supplied gradients.
            error(message('optimlib:fmincon2:GradOffTRR', linkToDoc))
        end
    end
end
...
if shiftedX0
    if verbosity >= 3
        if strcmpi(OUTPUT.algorithm,interiorPoint) 
            fprintf(getString(message('optimlib:fmincon2:ShiftX0StrictInterior')));
            fprintf('\n');
        else
            fprintf(getString(message('optimlib:fmincon2:ShiftX0ToBnds')));
            fprintf('\n');
        end
    end
end
...
case 'fun'
   try
      initVals.f = feval(funfcn{3},X,varargin{:});
        ...
        optim_ME = MException('optimlib:fmincon2:ObjectiveError', ...
            getString(message('optimlib:fmincon2:ObjectiveError')));
...
case 'fun_then_grad'
   ...
   try
      initVals.g = feval(funfcn{4},X,varargin{:});
        ...
        optim_ME = MException('optimlib:fmincon2:GradientError', ...
            getString(message('optimlib:fmincon2:GradientError')));
        userFcn_ME = addCause(userFcn_ME,optim_ME);
        rethrow(userFcn_ME)
   end
case 'fun_then_grad_then_hess'
   ...
   try
      HESSIAN = feval(funfcn{5},X,varargin{:});
        ...
        optim_ME = MException('optimlib:fmincon2:HessianError', ...
            getString(message('optimlib:fmincon2:HessianError')));            
        userFcn_ME = addCause(userFcn_ME,optim_ME);
        rethrow(userFcn_ME)
   end
otherwise
   error(message('optimlib:fmincon2:UndefinedCallType'));
end

% Check that the objective value is a scalar
if numel(initVals.f) ~= 1
   error(message('optimlib:fmincon2:NonScalarObj'))
end

% Check that the objective gradient is the right size
initVals.g = initVals.g(:);
if numel(initVals.g) ~= sizes.nVar
   error('optimlib:fmincon2:InvalidSizeOfGradient', ...
...
case 'fun'
    try
        ...
        if strcmpi('MATLAB:maxlhs',userFcn_ME.identifier)
                error(message('optimlib:fmincon2:InvalidHandleNonlcon'))
        else
            optim_ME = MException('optimlib:fmincon2:NonlconError', ...
                getString(message('optimlib:fmincon2:NonlconError')));
            userFcn_ME = addCause(userFcn_ME,optim_ME);
            rethrow(userFcn_ME)
        end
    end
...
case 'fun_then_grad'
    ...
    try
        ...
        optim_ME = MException('optimlib:fmincon2:NonlconFunOrGradError', ...
            getString(message('optimlib:fmincon2:NonlconFunOrGradError')));
        userFcn_ME = addCause(userFcn_ME,optim_ME);
        rethrow(userFcn_ME)
    end
case ''
...
if ~isempty( isoptimargdbl2('fmincon2', {'f','g','H','c','ceq','gc','gceq'}, ...
   initVals.f, initVals.g, HESSIAN, initVals.ncineq, initVals.nceq, initVals.gnc, initVals.gnceq) )
    error('optimlib:fmincon2:NonDoubleFunVal',getString(message('optimlib:commonMsgs:NonDoubleFunVal','fmincon2')));
end
...
if cgrow ~= sizes.nVar || cgcol ~= sizes.mNonlinIneq
   error(message('optimlib:fmincon2:WrongSizeGradNonlinIneq', sizes.nVar, sizes.mNonlinIneq))
end
if ceqgrow ~= sizes.nVar || ceqgcol ~= sizes.mNonlinEq
   error(message('optimlib:fmincon2:WrongSizeGradNonlinEq', sizes.nVar, sizes.mNonlinEq))
end

if diagnostics
   % Do diagnostics on information so far
   diagnose2('fmincon2',OUTPUT,flags.grad,flags.hess,flags.constr,flags.gradconst,...
      XOUT,sizes.mNonlinEq,sizes.mNonlinIneq,lin_eq,lin_ineq,l,u,funfcn,confcn);
end

% Create default structure of flags for finitedifferences2:
...
cleanupObj = setOptimFcnHandleOnWorkers2(UseParallel,funfcn,confcn); %#ok<NASGU>
...
if derivativeCheck && ...               % User wants to check derivatives...
   ...
   flags.gradconst && sizes.mNonlinEq+sizes.mNonlinIneq > 0) % nonlinear constraint function.
    validateFirstDerivatives2(funfcn,confcn,X, ...
...
if strcmpi(OUTPUT.algorithm,activeSet) % active-set
    ...
    [X,FVAL,LAMBDA,EXITFLAG,OUTPUT,GRAD,HESSIAN]=...
        nlconst2(funfcn,X,l,u,full(A),B,full(Aeq),Beq,confcn,options,defaultopt, ...
        finDiffFlags,verbosity,flags,initVals,problemInfo,optionFeedback,varargin{:});
elseif strcmpi(OUTPUT.algorithm,trustRegionReflective) % trust2-region-reflective
   ...
   elseif (strcmpi(funfcn{1}, 'fun_then_grad') || strcmpi(funfcn{1}, 'fungrad'))
      ...
      if ischar(Hstr) 
         if strcmpi(Hstr,'sparse(ones(numberofvariables))')
            ...
            error(message('optimlib:fmincon2:InvalidHessPattern'))
         end
      end
      checkoptionsize2('HessPattern', size(Hstr), n);
   end
   
   defaultopt.MaxIter = 400; defaultopt.MaxFunEvals = '100*numberofvariables'; defaultopt.TolX = 1e-6;
   defaultopt.Hessian = 'off';
   % trust2-region-reflective algorithm does not compute constraint
...
elseif strcmpi(OUTPUT.algorithm,interiorPoint)
    ...
    options = getSQPOptions2(options,defaultopt,sizes.nVar);
