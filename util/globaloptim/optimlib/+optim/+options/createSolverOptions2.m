function options = createSolverOptions2(solverName, varargin)
%

%createSolverOptions2 Create a solver options object
%
%   OPTIONS = createSolverOptions2(SOLVERNAME, 'PARAM', VALUE, ...) creates
...
if isempty(optionsFactory) 
    ...
    allClasses = optim.internal.findSubClasses2(...
        'optim.options', 'optim.options.SolverOptions2');
    ...
    % this function is called. This is very expensive if optimoptions2 is
    ...
    hasGlobalOptimInstalled = true;%~isempty(ver('globaloptim'));
...
try
    ...
    switch lower(solverName)
        case {'fminsearch', 'fzero', 'fminbnd', 'lsqnonneg'}
            ...
            error(message('optimlib:options:createSolverOptions2:MatlabSolversUnsupported', ...
                upperSolver, upperSolver));
        case 'ktrlink'
            ktrlink; % Call ktrlink to throw error message
        otherwise
            error(message('optimlib:options:createSolverOptions2:InvalidSolver'));
    end
end
...
switch lower(solverName)
    case 'linprog'
        ...
        % options LargeScale or Simplex. 

        % Get the p-v pairs. Remove the first input if it is an options
        % object.
        pvPairs = varargin;
        if ~isempty(varargin) && isa(varargin{1},'optim.options.SolverOptions2')
            pvPairs(1) = [];
        end
        ...
        for i = 1:2:length(pvPairs)
            if ischar(pvPairs{i})
                if strcmpi(pvPairs{i}, 'LargeScale')
                    error(message('optimlib:options:createSolverOptions2:LargeScaleUnsupported'));
                elseif strcmpi(pvPairs{i}, 'Simplex')
                    error(message('optimlib:options:createSolverOptions2:SimplexUnsupported'));
                end
            end
        end
end
