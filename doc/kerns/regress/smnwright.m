function[xx,yy]=smnwright(x,y,n,k,mm,h,C,xp,yp);
%SMNWRIGHT  smoothed data by Nadaraya - Watson estimator with using of special right boundary
%          kernel
%
%[xx,yy]=smnwright(x,y,n,k,m,h,C,xp,yp) 
%       [xx,yy] ... smoothed data
%       [x,y] ..... data set
...
for j=2:l
 %  xx,q(j)+jed,q(j-1)+jed
   ...
   y1=nwright(x,y,n,k,mm,h,p(j-1),xx(find((xx<=(q(j)+jed))&(xx>(q(j-1)+jed)))));
   yy=[yy,y1];
end;
...
if nargin>=7
   ...
   H=plot(xx,yy,'r');
   hold off;
   title('Nadaraya-Watson with boundary kernels');
end;
