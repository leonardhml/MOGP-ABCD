function[hmin,gcvmin,hh,gcv,trs]=rcvpch(x,y,n,k,mm,C);
%RCVPCH  estimation of optimal bandwidth for Pristley - Chao estimator
%        by using Rice's penalizing function
%
%[hmin,acvmin]=rcvpch(x,y,n,k,m,C) 
...
while h<hx
   ...
   gcv=[gcv,gcv1];hh=[hh,h];tr=1/(1-2*tr/nn);
   trs=[trs,tr];
   end;
...
xlabel('h');
ylabel('RCV');
title('Rice error function for PCH estimator');
figure(C+1);
plot(hh,trs,'b');
title('Rice penalizing function for PCH estimator');
end;
