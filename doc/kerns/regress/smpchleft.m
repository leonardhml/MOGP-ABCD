function[xx,yy]=smpchleft(x,y,n,k,mm,h,C,xp,yp);
%SMPCHLEFT  smoothed data by Pristley - Chao estimator with using of special left boundary
%           kernel
%
%[xx,yy]=smpchleft(x,y,n,k,m,h,C,xp,yp) 
%       [xx,yy] ... smoothed data
%       [x,y] ..... data set
...
l=length(q);
for j=2:l
   y1=pchleft(x,y,n,k,mm,h,p(j-1),xx(find((xx<=q(j))&(xx>q(j-1)))));
   yy=[yy,y1];
end;
