function[hmin,gcvmin,hh,gcv,trs]=scvll(x,y,n,k,mm,C);
%SCVLL  estimation of optimal bandwidth for local - linear estimator
%       by using Shibata's penalizing function
%
%[hmin,acvmin]=scvll(x,y,n,k,m,C) 
...
xlabel('h');
ylabel('SCV');
title('Shibata error function for LL estimator');
figure(C+1);
plot(hh,trs,'b');
title('Shibata penalizing function for LL estimator');
end;
