function[xx,yy]=smllright(x,y,n,k,mm,h,C,xp,yp);
%SMLLRIGHT  smoothed data by local - linear estimator with using of special right boundary
%          kernel
%
%[xx,yy]=smllright(x,y,n,k,m,h,C,xp,yp) 
%       [xx,yy] ... smoothed data
%       [x,y] ..... data set
...
l=length(q);
for j=2:l
   y1=llright(x,y,n,k,mm,h,p(j-1),xx(find((xx<=(q(j)+jed))&(xx>(q(j-1)+jed)))));
   yy=[yy,y1];
end;
