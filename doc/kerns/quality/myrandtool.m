function myrandtool(action,flag)
%MYRANDTOOL Demonstration of many random number generators.
%   MYRANDTOOL creates a histogram of random samples from many
...
%   $Revision: 2.32.6.16 $  $Date: 2008/08/20 23:04:46 $
%
%   This is only the copy of RANDTOOL changed for my applicatons
if nargin < 1
    action = 'start';
end
...
if strcmp(action,'start'),
    ...
    for idx = 1:3
        ...
        ud.pfhndl(idx) = uicontrol('Style','edit','Units','normalized',...
            ...
            'CallBack',['myrandtool(''setpfield'',',nstr,')'],...
        ...
        ud.hihndl(idx) = uicontrol('Style','edit','Units','normalized',...
            ...
            'BackgroundColor','white',...
            'CallBack',['myrandtool(''setphi'',',nstr,')'],...
        ...
        ud.lohndl(idx) = uicontrol('Style','edit','Units','normalized',...
            ...
            'BackgroundColor','white',...
            'CallBack',['myrandtool(''setplo'',',nstr,')'],...
        ...
        ud.pslider(idx) = uicontrol('Style','slider','Units','normalized',...
            ...
            'Callback',['myrandtool(''setpslider'',',nstr,')'],...
...
if badval
    ...
    uiwait(warndlg(wmsg, 'myrandtool', 'modal'))
...
try
    ...
    MEbase = MException('stats:myrandtool:GeneratorError',...
...
GG=get(findobj(0,'name','SIMULATION OF MODEL VARIABLES'),'userdata');
ktera=GG(1);
switch ktera
    case 0
        def{1}=[def{1},'0'];
...
evalin('base','cisti;save ppp;clear all;');
[hdialog,ok_pressed]=export2wsdlg(label, def, item, 'Export to Workspace');
if ok_pressed
    G=get(findobj(0,'tag','defprom'),'userdata');
    chdef=char(evalin('base','who'));
    evalin('base',[chdef,'=',chdef,'(:)'';']);
    evalin('base',['save puvprom ',chdef,' -append']);
    switch ktera
        case 0
            prom=get(G(1),'String');
            newprom=pridej(prom,chdef);
            set(G(1),'String',newprom,'Value',1,'Enable','on');
            set(GG(2),'Enable','off');
            x_def=ud.random_numbers(:)';
            save puvprom x_def -append;
            clr='cyan';
        case 1
            prom=get(G(2),'String');
            newprom=pridej(prom,chdef);
            set(G(2),'String',newprom,'Value',1,'Enable','on');
            set(GG(3),'Enable','off');
            y_def=ud.random_numbers(:)';
            save puvprom y_def -append;
            clr='red';
    end
    of1=get(GG(2),'Enable');
    of2=get(GG(3),'Enable');
    figure(findobj(0,'name','SIMULATION OF MODEL VARIABLES'));
    hold on;
    minrn = min(ud.random_numbers);
    maxrn = max(ud.random_numbers);
    if ud.dists(popupvalue).discrete
        edges = minrn-0.5:1:(maxrn+.5); % bins centered on the integers
    else
        samples = str2double(get(ud.samples_field,'String'));
    ...
    counts = histc(ud.random_numbers,edges)/length(ud.random_numbers);
    hbar = bar(edges,counts,clr);
    set(hbar,'Tag','dbar');
%    popupvalue = get(ud.popup,'Value');
%    xrange = getxdata(popupvalue,ud);
%    set(gca,'XLim',xrange, 'XTickMode','auto', 'XTickLabelMode','auto');
    text(-0.08, 0.45,'Counts','Unit','Normalized','FontUnits','normalized','rotation', 90);
    text(0.45,-0.06,'Values','Unit','Normalized','FontUnits','normalized');
    if strcmp(of1,'off')&&strcmp(of2,'off')
        set(GG(4),'Enable','on');
        set(G(7),'Enable','on');
        legend(gca,'G_0 sample','G_1 sample');
    end
end
evalin('base','load ppp;delete ppp.mat;');
